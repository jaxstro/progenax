...
def _clumpy(n, seed, k=8, spread=0.06):
    rng = np.random.default_rng(seed)
    c = rng.uniform(-1, 1, (k, 3)); c /= np.maximum(np.linalg.norm(c, axis=1, keepdims=True), 1e-9)
...
def fig_plane(output_dir):
    ...
    ax.plot(sgrid, 0.8 * sgrid, "--", color="0.4", lw=1.3, label=r"$Q=0.8$ (CW04 divide)")
    ...
    for name, gen, col in CONFIGS:
        ...
        ax.plot(sb, mb, "o", color=col, ms=8, mec="white", mew=0.6,
...
def fig_ladder(output_dir):
    ...
    for name, gen, col in CONFIGS:
        ...
        names.append(name); Qmean.append(Qs.mean()); Qstd.append(Qs.std()); cols.append(col)
    ...
    ax.bar(xpos, Qmean, yerr=Qstd, color=cols, alpha=0.85, capsize=3,
    ...
    ax.set_ylim(0.3, 1.0)
    ax.text(0.02, 0.97, "concentrated", transform=ax.transAxes, fontsize=7.5,
            color="0.35", va="top")
    ax.text(0.02, 0.10, "substructured", transform=ax.transAxes, fontsize=7.5, color="0.35")
...
def fig_approx_vs_exact(output_dir):
    ...
    for name, gen, col in CONFIGS:
        ...
        ex.append(e); ap.append(a); cols.append(col); names.append(name)
    ...
    print(f"  faithful (Q<=0.85, |diff|<0.06): {faithful}  -> {'PASS' if faithful else 'FAIL'}")
    ...
    ax.text(0.45, 1.05, "faithful regime\n(substructure)", fontsize=7.5, color="0.35", va="top")
...
def fig_gradient(output_dir):
    ...
    dirs = rng.normal(0, 1, (500, 3)); dirs /= np.linalg.norm(dirs, axis=1, keepdims=True)
    u = jnp.asarray(rng.uniform(0.02, 1.0, 500)); D = jnp.asarray(dirs)
    ...
    fd = np.array([float((q_of_p(float(p) + 5e-4) - q_of_p(float(p) - 5e-4)) / 1e-3) for p in ps])
    ...
    print(f"  q(p): {qvals[0]:.2f} -> {qvals[-1]:.2f} (concentration raises Q; all AD>0: {correct_sign})")
    print(f"  AD-vs-FD rel: median={rel_med:.2e}, max={rel_max:.2e} (kNN cell-boundary)  "
          f"-> {'PASS' if passed else 'FAIL'}")

    fig, ax = plt.subplots(figsize=(3.9, 3.4))
    ax.plot(ps, ad, "-", color=OI["blue"], lw=1.8, label="autodiff", zorder=2)
    ax.plot(ps, fd, "o", color=OI["vermilion"], ms=4.5, mfc="none", mew=1.2,
            label="finite diff", zorder=3)
    ax.set_xlabel(r"concentration exponent $p$  ($r=u^p$)")
    ax.set_ylabel(r"$\partial\,q_{\rm approx} / \partial p$")
    ax.legend(loc="best")
    ax.text(0.5, 0.05, rf"median rel $={rel_med:.0e}$ (kNN; finite, correct sign)",
...
def fig_n_convergence(output_dir):
    ...
    fig, ax = plt.subplots(figsize=(3.9, 3.4))
    ok = True
    for name, gen, col, ref in [("uniform", _uniform_sphere, OI["blue"], 0.79),
                                ("clumpy", _clumpy, OI["vermilion"], None)]:
        mean, std = [], []
        for N in Ns:
            Qs = np.array([compute_q_parameter(gen(N, s)) for s in range(5)])
            mean.append(Qs.mean()); std.append(Qs.std())
        mean, std = np.array(mean), np.array(std)
        ax.errorbar(Ns, mean, yerr=std, fmt="o-", color=col, ms=4.5, capsize=2,
                    lw=1.2, label=name)
        big = mean[np.array(Ns) > 100]
        spread = (big.max() - big.min()) / big.mean()
        ok = ok and spread < 0.25
        print(f"  {name:8s}: Q(N>100) spread = {spread:.1%}  "
