...
OBS_PERIOD_YR = {
    "Mercury": 0.2408, "Venus": 0.6152, "Earth": 1.0, "Mars": 1.881,
    "Jupiter": 11.862, "Saturn": 29.457, "Uranus": 84.02, "Neptune": 164.79,
}
...
def fig_two_body(output_dir):
    ...
    dE = abs((E1 - E0) / E0); dL = float(jnp.linalg.norm(L1 - L0))
    E_exact = -G * M1 * M2 / (2 * a)
    e_match = abs(E0 - E_exact) < 1e-12
    closed = closure < 1e-4; cons = dE < 1e-5 and dL < 1e-6
    print(f"  E = {E0:.6f} (exact -G m1 m2/2a = {E_exact:.6f})  -> {'PASS' if e_match else 'FAIL'}")
    print(f"  closure after 1 period = {closure:.2e} (tol 1e-4)  -> {'PASS' if closed else 'FAIL'}")
    print(f"  dE/E = {dE:.2e} (tol 1e-5), d|L| = {dL:.2e} (tol 1e-6)  "
    ...
    axA.plot(rel[0, 0], rel[0, 1], "o", color=OI["vermilion"], ms=6, mec="white",
             label="start = end")
    axA.set_xlabel(r"$x$ [code]"); axA.set_ylabel(r"$y$ [code]")
    axA.set_aspect("equal"); axA.legend(loc="upper right", fontsize=7)
    axA.text(0.04, 0.05, rf"$e={e}$, closure $={closure:.0e}$", transform=axA.transAxes,
    ...
    bars = {r"$|E-E_{\rm exact}|$": abs(E0 - E_exact) / 1e-12,
            r"$\Delta E/E$": dE / 1e-5, r"$\Delta|L|$": dL / 1e-6,
            "closure": closure / 1e-4}
    axB.bar(range(len(bars)), [max(v, 1e-3) for v in bars.values()],
            color=[OI["green"] if v < 1 else OI["vermilion"] for v in bars.values()],
            edgecolor="white")
    axB.set_yscale("log"); axB.set_ylim(1e-3, 2)
    axB.set_xticks(range(len(bars))); axB.set_xticklabels(list(bars.keys()), fontsize=7)
...
def fig_figure_eight(output_dir):
    ...
    n = 200_000
    traj, pf, vf = _verlet_traj(ic.positions, ic.velocities, ic.masses, 1.0, ic.period, n)
    closure = float(jnp.max(jnp.linalg.norm(pf - ic.positions, axis=1)))
    L = float(jnp.linalg.norm(_ang_mom(ic.positions, ic.velocities, ic.masses)))
    closed = closure < 1e-6; zeroL = L < 1e-10
    print(f"  |L| = {L:.2e} (tol 1e-10)  -> {'PASS' if zeroL else 'FAIL'}")
    print(f"  closure after 1 period = {closure:.2e} (tol 1e-6)  -> {'PASS' if closed else 'FAIL'}")
    ...
    for i, c in enumerate(cols):
        ax.plot(ic.positions[i, 0], ic.positions[i, 1], "o", color=c, ms=8,
                mec="white", mew=0.8, zorder=3, label=f"body {i+1} (start)")
    ax.plot(0, 0, "+", color=OI["black"], ms=9, mew=1.3, zorder=2)
    ax.set_xlabel(r"$x$ [code]"); ax.set_ylabel(r"$y$ [code]")
    ax.set_aspect("equal"); ax.legend(loc="upper right", fontsize=6.8)
    ax.text(0.04, 0.06, rf"$|L|={L:.0e}$" + "\n" + rf"closure $={closure:.0e}$",
...
def fig_kepler_third(output_dir):
    ...
    print(f"  Kepler III T(a) vs observed period: max rel = {np.max(rel):.2e} "
    ...
    axA.loglog(aa**3, (2 * np.pi * np.sqrt(aa**3 / G_PLANET)) ** 2, "-",
               color=OI["black"], lw=1.4, label=r"$T^2 = \frac{4\pi^2}{GM_\odot}a^3$")
    axA.loglog(a**3, T_obs**2, "o", color=OI["vermilion"], ms=6, mec="white",
               mew=0.6, label="planets (observed)")
    for nm, ai, ti in zip(names, a, T_obs):
        axA.annotate(nm[:3], (ai**3, ti**2), textcoords="offset points",
                     xytext=(4, -2), fontsize=5.5, color="0.4")
    axA.set_xlabel(r"$a^3$ [AU$^3$]"); axA.set_ylabel(r"$T^2$ [yr$^2$]")
    axA.legend(loc="upper left", fontsize=7)
    panel_label(axA, "(a)", loc="lower right")

    axB.bar(range(len(names)), np.maximum(rel, 1e-6), color=OI["blue"], edgecolor="white")
    axB.axhline(1e-2, color=OI["vermilion"], ls="--", lw=1.2, label=r"$1\%$")
    axB.set_yscale("log"); axB.set_ylim(1e-6, 3e-2)
...
def fig_harmonic(output_dir):
    ...
    ic = harmonic_oscillator(amplitude=A_amp, omega=omega, phase=phase, mass=m, dimension="1D")
    # analytic solution over one period; integrate the SHO ODE for comparison
    T = 2 * np.pi / omega
    t = np.linspace(0, T, 400)
    x_an = A_amp * np.cos(omega * t + phase)
    v_an = -A_amp * omega * np.sin(omega * t + phase)

    # integrate x'' = -omega^2 x with velocity-Verlet from the IC
    x0 = float(ic.positions[0, 0]); v0 = float(ic.velocities[0, 0])
    n = 4000; dt = T / n
    xs, vs = [x0], [v0]
    x, v = x0, v0
    for _ in range(n):
    ...
    cons = dE < 1e-4
    print(f"  IC x(0)=A cos(phi): {x0:.6f} vs {A_amp*np.cos(phase):.6f}  "
          f"-> {'PASS' if ic_match else 'FAIL'}")
    print(f"  integrated vs analytic x(t): max dev = {max_dev:.2e} (tol 1e-3)  "
          f"-> {'PASS' if max_dev < 1e-3 else 'FAIL'}")
    print(f"  E = 0.5 m omega^2 A^2 = {E_exact:.4f}, max dE/E = {dE:.2e} (tol 1e-4)  "
          f"-> {'PASS' if cons else 'FAIL'}")

    fig, (axA, axB) = plt.subplots(1, 2, figsize=(7.2, 3.0))
    axA.plot(t, x_an, "-", color=OI["black"], lw=1.6, label=r"analytic $A\cos(\omega t+\phi)$")
    axA.plot(t_int[::120], x_int[::120], "o", color=OI["vermilion"], ms=4, mfc="none",
             mew=1.1, label="Verlet")
    axA.set_xlabel(r"$t$"); axA.set_ylabel(r"$x(t)$")
    axA.legend(loc="upper right", fontsize=7)
    panel_label(axA, "(a)", loc="upper left")

    axB.plot(t_int, E, "-", color=OI["blue"], lw=1.4)
    axB.axhline(E_exact, color=OI["black"], ls="--", lw=1.0,
                label=rf"$\frac{{1}}{{2}}m\omega^2A^2={E_exact:.2f}$")
    axB.set_xlabel(r"$t$"); axB.set_ylabel(r"energy $E$")
    axB.set_ylim(E_exact * (1 - 1e-3), E_exact * (1 + 1e-3))
    axB.legend(loc="upper right", fontsize=7)
    axB.text(0.5, 0.1, rf"max $\Delta E/E={dE:.0e}$", transform=axB.transAxes,
...
def fig_figure_eight_adversarial(output_dir):
    ...
    # canonical
    traj_c, pf_c, _ = _verlet_traj(ic.positions, ic.velocities, ic.masses, 1.0, ic.period, n)
    ...
    print(f"  canonical:  |L|={L_c:.2e}, closure={clos_c:.2e}  -> "
          f"{'PASS (closes, L=0)' if canon_ok else 'FAIL'}")
    print(f"  spurious-spin: |L|={L_w:.2e}, closure={clos_w:.2e}  -> "
...
def main():
    ...
    print("=" * 70)
    print("  ALL ANALYTICAL VALIDATION FIGURES PASS" if all_ok
