...
def main():
    print("\n" + "=" * 70)
    print("ANISOTROPIC MULTI-MASS LIMEPY SAMPLER VALIDATION (beta(r), sigma_r/t, virial)")
    print("=" * 70)
    ...
    for sd in range(N_SEED):
        ...
        for j, mj in enumerate([float(M_J[0]), float(M_J[1])]):
            ...
            for i, (lo, hi) in enumerate(zip(edges[:-1], edges[1:])):
                ...
                if m.sum() < 30:
                    continue
                beta_sd[j][i].append(1.0 - v_t2[m].mean() / (2.0 * (v_r[m] ** 2).mean()))
                if j == 0:
    ...
    def _mean_sem(rows):
        ...
        sem = np.array([np.std(x) / np.sqrt(len(x)) if len(x) > 1 else np.nan for x in rows])
        n = np.array([len(x) for x in rows])
        return mean, sem, n

    # (a) seed-averaged beta(r): sampled (mean +/- sem) vs analytic DF, per component
    print(f"  seed-averaged beta(r) [{N_SEED} seeds x {N_STARS}]:  r/rt  sampled+/-sem   DF")
    for j, (col, lab) in enumerate([(LCOL, "light"), (HCOL, "heavy")]):
        ...
        axA.errorbar(centers / rt, bmean, yerr=bsem, ls="none", marker="o", ms=4, mfc="white",
                     color=col, capsize=2, label=f"{lab} (sampled)")
        if j == 0:
            for x, bm, bs, bd in zip(centers / rt, bmean, bsem, b_df):
                print(f"                                  {x:.2f}  {bm:+.3f}+/-{bs:.3f}   {bd:+.3f}")
    axA.axhline(0.0, color="0.6", ls="--", lw=1.0)
    axA.set_xlabel(r"$r/r_t$"); axA.set_ylabel(r"anisotropy $\beta(r)$")
    ...
    axB.errorbar(centers / rt, srm, yerr=srs, ls="none", marker="o", ms=4, mfc="white",
                 color=OI["blue"], capsize=2, label=r"$\sigma_r$ (samp.)")
    axB.plot(centers / rt, st_df, color=OI["orange"], lw=1.6, label=r"$\sigma_t$ (DF)")
    axB.errorbar(centers / rt, stm, yerr=sts, ls="none", marker="s", ms=4, mfc="white",
                 color=OI["orange"], capsize=2, label=r"$\sigma_t$ (samp.)")
    axB.set_xlabel(r"$r/r_t$"); axB.set_ylabel(r"$\sigma_{r,t}$ (light)")
    ...
    for d in DELTAS:
        ...
        Qg.append(np.mean(qs)); Qse.append(np.std(qs) / np.sqrt(len(qs)))
        print(f"    delta={d:.1f}  Q={np.mean(qs):.3f} +/- {Qse[-1]:.3f}")
    axC.errorbar(DELTAS, Qg, yerr=Qse, marker="o", ms=5, lw=1.6, color=OI["green"], capsize=2)
    axC.axhline(0.5, color="0.6", ls="--", lw=1.0)
    axC.set_ylim(0.44, 0.56)
    axC.set_xlabel(r"equipartition $\delta$"); axC.set_ylabel(r"global $Q = T/|V|$")
    ...
    for r_a, col in zip([8.0, 6.0, 5.0, 4.0],
        ...
        axD.plot(fr, b, color=col, lw=1.6, label=fr"$r_a={r_a:.0f}$")
    axD.axhline(0.0, color="0.6", ls="--", lw=1.0)
    axD.set_xlabel(r"$r/r_t$"); axD.set_ylabel(r"$\beta_{\rm light}(r)$ (DF)")
    ...
    print(f"  seed-avg beta_light(r)=DF, resolved bins (|d|<0.04, {n_checked} bins): "
