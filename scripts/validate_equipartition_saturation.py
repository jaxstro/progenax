...
def central_sigma_quadrature(W_j, w_j, g, n_u=400):
    ...
    return w_j * jnp.sqrt(jnp.trapezoid(u**4 * E, u) / jnp.trapezoid(u**2 * E, u) / 3.0)
...
def central_sigma_closed_form(W_j, w_j, g):
    ...
    """
    return w_j * jnp.sqrt(
        lowered_exponential(g + 2.5, W_j) / lowered_exponential(g + 1.5, W_j)
    )
...
def ln_sigma_exact(lnm, bar_m, W0, g, delta=DELTA):
    ...
    W = mu ** (2.0 * delta) * W0
    return -delta * jnp.log(mu) + 0.5 * (
        jnp.log(lowered_exponential(g + 2.5, W))
...
def make_figure(results):
    ...
    fig, axes = plt.subplots(
        1, len(results), figsize=(9.6, 3.4), sharey=False, gridspec_kw=dict(wspace=0.30)
    )
    for k, res in enumerate(results):
        ...
        sig_exact = np.exp(
            np.asarray(
                jax.vmap(lambda lm: ln_sigma_exact(lm, bar_m, W0, G_TRUNC))(jnp.log(mm))
            )
        )
        ax.plot(
            mm, sig_exact, color=OI["black"], lw=1.6, label=r"exact $\hat\sigma(m)$"
        )
        ...
        ax.plot(
            ...
            mec=OI["black"],
            mew=0.9,
            label=r"model $\hat\sigma_{1d,j0}$ (20 bins)",
            zorder=5,
        )
...
def main():
    print("\n" + "=" * 88)
    print(
        "DERIVED-m_eq EQUIPARTITION SATURATION (Bianchini et al. 2016 App. A)"
        " -- FIT-FREE"
    )
    print(
        f"  {N_BINS} log bins on [{M_RANGE[0]}, {M_RANGE[1]}] Msun, equal M_j; "
        f"delta={DELTA}, g={G_TRUNC}; m_eq = m-bar (g+5/2)(g+7/2)/W0"
    )
    print(
        "  All gates are exact statements (closed form + autodiff); nothing is fitted."
    )
    print("=" * 88)
    ...
    for res in results:
        ...
        for label, val, gate in checks:
            ...
            all_ok &= ok
            print(
                f"  {res['W0']:>4.0f} {label:<38} {val:>10.2e} {gate:>9.0e} "
    ...
    print(
        f"  {'W0':>4} {'m-bar':>7} {'derived m_eq':>13} {'m/(2 eta)':>10} "
    ...
    print(
        "      (-> 1 as mu -> 0: App. A2<->A3 is the exact leading order."
    ...
    for res in results:
        print(
            f"  {res['W0']:>4.0f}"
            + "".join(f"  {loc / res['meq_derived']:<9.4f}" for loc in res["meq_local"])
        )
    ...
    for res in results:
        checks = [
            ...
            (
                f"|eta - 1/2| at mu={MU_HIGH:g} (deep well)",
                res["eta_half_dev"],
                GATE_ETA_HALF,
            ),
        ]
        ...
        all_ok &= mono
        print(
            f"  {res['W0']:>4.0f} {'eta(m) strictly monotone increasing':<38} "
