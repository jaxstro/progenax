...
def _count_iters(m_j, M_j, W0, delta):
    ...
    while iters < N_ITER and float(residual) > TOL:
        alpha = _alpha_map(
            alpha, m_j, f_target, W0, G_MODEL, delta, 300.0, 2000, None, 0.0, "table"
        )
        f_real = _realized_fractions(
            alpha, m_j, W0, G_MODEL, delta, 300.0, 2000, None, 0.0, "table"
        )
...
def main() -> int:
    ...
    print(
        f"date {datetime.date.today().isoformat()}  |  platform {sys.platform}  "
        f"|  x64 {jax.config.jax_enable_x64}"
    )
    print(
        f"N_COMP={N_COMP}  M_RANGE={M_RANGE}  g={G_MODEL}  tol={TOL:g}  "
        f"n_iter cap={N_ITER}  warm calls={N_WARM} (median)"
    )
    print(
        f"timed path: find_alpha_for_masses directly; grad wrt delta of "
    ...
    for alpha_imf, delta, W0 in CORNERS:
        ...
        rows.append(
            (alpha_imf, delta, W0, iters, residual, warm_ms, status, residual_match)
        )

        print(
            f"  corner alpha={alpha_imf:.1f} delta={delta:.1f} W0={W0:.0f}: "
    ...
    if any_fail:
        print(
            "  FAIL: a box corner did not converge below the 2e-3 residual gate "
