...
def fig_gc_anchors(output_dir):
    ...
    for n, p, c, d in zip(names, pub, comp, dev):
        print(
            f"  {n:9s} published {p:.2f}  computed {c:.3f}  |dev|={d:.3f} "
            f"(tol 0.20)  -> {'PASS' if d < 0.20 else 'FAIL'}"
        )
    print(
        f"  NGC 7078 most top-heavy: {most_top_heavy}  "
    ...
    ax.fill_between(
        ...
        alpha=0.10,
        zorder=0,
        label=r"$\pm0.20$ (Table 1 scatter)",
    )
...
def fig_fundamental_plane(output_dir):
    ...
    dominates = ratio > 3.0
    print(
        f"  1-dex response: d(alpha3)/d(log rho)={d_rho:+.3f}, "
    ...
    cs = ax.contour(
        lr, feh, A3, levels=[0.8, 1.0, 1.2, 1.6, 2.0], colors="white", linewidths=0.7
    )
    ax.clabel(cs, inline=True, fontsize=6, fmt="%.1f")
    for n, fe, rho, _ in GCS:
        ax.plot(
            np.log10(rho), fe, "o", color=OI["vermilion"], ms=6, mec="white", mew=0.8
        )
        ax.annotate(
            n.replace("NGC ", ""),
            (np.log10(rho), fe),
    ...
    ax.text(
        0.04,
        0.06,
        rf"density $\Rightarrow$ {ratio:.0f}$\times$ metallicity",
        transform=ax.transAxes,
        fontsize=7,
        color="white",
    )
...
def fig_lowmass_slopes(output_dir):
    ...
    feh = np.linspace(-2.5, 0.5, 200)
    a1 = np.array(
        jax.vmap(lambda f: lowmass_slopes_metallicity(f)[0])(jnp.asarray(feh))
    )
    a2 = np.array(
        jax.vmap(lambda f: lowmass_slopes_metallicity(f)[1])(jnp.asarray(feh))
    )
    ...
    for fe, a1e, a2e in TABLE4:
        ...
        worst = max(worst, d)
        print(
            f"  FeH={fe:+.1f}: a1={float(c1):.3f}(pub {a1e}) "
            f"a2={float(c2):.3f}(pub {a2e})  max|dev|={d:.4f}"
        )
    ok = worst < 0.02
    print(
        f"  worst Table-4 deviation {worst:.4f} (tol 0.02)  "
    ...
    ax.plot(
        ...
        color=OI["blue"],
        lw=1.8,
        label=r"$\alpha_1$ ($0.08$-$0.5\,M_\odot$)",
    )
    ax.plot(
        feh,
        a2,
    ...
    ax.set_ylabel(r"low-mass slope $\alpha$")
    ax.legend(loc="upper left", fontsize=7)
    ax.text(
        0.96,
        0.05,
        r"metal-poor $\Rightarrow$ bottom-light",
        transform=ax.transAxes,
        ha="right",
        fontsize=7,
        color="0.4",
...
def _alpha3_marks_preerratum(lr, feh):
    ...
    line = (
        MARKS_COEFFICIENTS["alpha3_slope"] * xh + MARKS_COEFFICIENTS["alpha3_intercept"]
    )
...
def fig_marks_vs_jerabkova(output_dir):
    ...
    for feh, col in [(-2.0, OI["purple"]), (-1.0, OI["green"]), (0.0, OI["orange"])]:
        am = np.array(
            alpha3_marks_plane(jnp.asarray(lr), jnp.asarray(feh))
        )  # erratum (-0.87)
    ...
    print(
        f"  erratum-corrected Marks vs Jeřábková_rho: max|gap| = {max_gap:.3f} "
        f"(tol 0.05)  -> {'PASS' if agree else 'FAIL'}"
    )
    print(
        f"  both monotone top-heavy with density: {both_mono}  "
        f"-> {'PASS' if both_mono else 'FAIL'}"
    )
    print(
        "  (the pre-erratum +0.87 typo would spuriously hold alpha3=2.3 out to "
        "x_hat=+0.87; corrected threshold is -0.87)"
    )
    ...
    ax.text(
        0.96,
        0.94,
        r"corrected Marks $\equiv$ Jeřábková" + "\n"
...
def fig_gradient_validation(output_dir):
    ...
    specs = [
        (
            "FeH",
            r"[Fe/H]",
            r"$\partial\alpha_3/\partial$[Fe/H]",
            lambda fe: alpha3_marks_plane(
                jnp.log10(jnp.asarray(40.0)), fe, smooth=True
            ),
            np.linspace(-2.4, 0.4, 13),
            1e-4,
        ),
        (
            "logrho",
            r"$\log_{10}\rho_6$",
            r"$\partial\alpha_3/\partial\log\rho$",
            lambda lr: alpha3_marks_plane(lr, jnp.asarray(-1.5), smooth=True),
            np.linspace(0.2, 2.8, 13),
            1e-4,
        ),
    ]
    ...
    for ax, (key, xlab, ylab, f, xs, h), tag in zip(axes, specs, "ab"):
        ...
        print(
            f"  d alpha3/d{key}: max rel err {np.max(rel):.2e}  "
...
def main():
    ...
    print("=" * 70)
    print(
        "  ALL ENVIRONMENT-IMF VALIDATION FIGURES PASS"
        if all_ok
