...
def _engine_b(theta):
    ...
    m = MultiComponentCluster.from_density_profiles(
        [PlummerProfile(r_h=r_h), EFFProfile(a=EFF_A, gamma=EFF_GAMMA, r_t=EFF_RT)],
        jnp.stack([t, 1.0 - t]),
        m_j=jnp.array(M_J),
...
def build_truth_data(key=None):
    ...
    beta_se = jnp.where(
        weight > 0, (1.0 + jnp.abs(beta_hat)) / jnp.sqrt(jnp.maximum(n, 1.0)), 0.0
    )

    # Per-(comp, bin) mean radius (for placing the figure's data points).
    bin_ids = di._bin_index(r, R_EDGES)
    r_sum, r_cnt = di._grouped_bin_sums(
        r, cid, bin_ids, sig_hat.shape[0], R_EDGES.shape[0] - 1
    )
...
def self_consistency_check(data):
    ...
    print(
        f"{'chan':>6} {'cmp':>3} {'bin':>3} {'data':>9} {'pred':>9} "
        f"{'se':>8} {'dev/se':>8}"
    )
    max_dev = 0.0
...
def make_figure(data, theta_hat, sigma_theta, cov_theta, out_dir):
    ...
    for j, (ax, lab) in enumerate([(ax_h, "(a)"), (ax_c, "(b)")]):
        ...
        ax.plot(
            rg, sig_fit[j], "-", color=colors[j], lw=1.6, zorder=2, label="best fit"
        )
        ax.plot(
            rg,
            sig_true[j],
    ...
    ax_b.plot(
        ...
        alpha=0.7,
        zorder=1,
        label=rf"truth $r_a={R_A_TRUE}$",
    )
    ax_b.axhline(
        0.0, color=ps.OI["green"], lw=0.8, ls="--", alpha=0.6, label="core (isotropic)"
    )
    ...
    cap = (
        rf"$\hat t={t_hat:.3f}\pm{float(sigma_theta[0]):.3f},\ "
...
def main():
    ...
    print("=" * 72)
    print(
        f"truth: t={T_TRUE}, r_a={R_A_TRUE}, r_h={R_H_TRUE}; "
    ...
    for j in range(J):
        ...
        resolved.append(nb)
        print(
            f"  component {j}: {[int(x) for x in n[j]]}  "
    ...
    print(
        f"grad norm at z_hat = {float(jnp.linalg.norm(g_hat)):.4e} "
    ...
    print(
        f"  realizability: min(f_min_j) = {min_fmin:.4e} "
    ...
    for j, nb in enumerate(resolved):
        print(
            f"  occupancy comp {j}: {nb} resolved sigma bins "
    ...
    print(
        f"Fisher correlations: rho(r_a, r_h) = {rho_rarh:+.3f}, "
