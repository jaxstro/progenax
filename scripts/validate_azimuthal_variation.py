...
def fig_vs_clumpiness(output_dir):
    ...
    for f in fracs:
        v = [
            compute_azimuthal_variation(
                _azimuthal_clumps(N, s, frac=float(f)), n_bins=N_BINS
            )
            for s in range(6)
        ]
    ...
    print(
        f"  clumpy (frac=0.9): {mean[-1]:.3f};  monotonic: {monotone}  "
    ...
    ax.axhline(
        floor, color="0.5", ls=":", lw=1.1, label=r"Poisson floor $\sqrt{n_b/N}$"
    )
    ax.errorbar(
        fracs,
        mean,
        ...
        capsize=2,
        lw=1.4,
        label=r"$\sigma_\Sigma/\langle\Sigma\rangle$",
    )
...
def fig_vs_cw04q(output_dir):
    ...
    for f in np.linspace(0.0, 0.85, 7):
        for s in range(4):
            ...
            sig.append(compute_azimuthal_variation(p))
            Q.append(compute_q_parameter(p))
    sig, Q = np.array(sig), np.array(Q)
    corr = float(np.corrcoef(sig, Q)[0, 1])
    passed = corr < -0.5
    print(
        f"  corr(sigma_Sigma, Q) = {corr:.2f} (expect strongly negative)  "
        f"-> {'PASS' if passed else 'FAIL'}"
    )

    fig, ax = plt.subplots(figsize=(3.9, 3.4))
    ax.scatter(Q, sig, c=sig, cmap="viridis", s=24, edgecolor="white", linewidth=0.3)
...
def fig_histograms(output_dir):
    ...
    sig_sm = compute_azimuthal_variation(sm)
    sig_cl = compute_azimuthal_variation(cl)
    print(f"  smooth sigma={sig_sm:.3f}, clumpy sigma={sig_cl:.3f}")

    fig, (axA, axB) = plt.subplots(
        1, 2, subplot_kw={"projection": "polar"}, figsize=(6.6, 3.3)
    )
    for ax, pos, col, title, sg in [
        (axA, sm, OI["blue"], "smooth", sig_sm),
        ...
        ax.set_title(
            rf"{title}: $\sigma_\Sigma/\langle\Sigma\rangle={sg:.2f}$", fontsize=9
        )
        ax.set_yticklabels([])
...
def main():
    ...
    print("=" * 70)
    print(
        "  ALL AZIMUTHAL-VARIATION FIGURES PASS" if all_ok else "  SOME FIGURES FAILED"
    )
    print("=" * 70)
