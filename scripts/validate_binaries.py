...
def fig_kepler_orbits(output_dir):
    ...
    print(
        f"  Kepler III: max rel(T, 2pi sqrt(a^3/GM)) = {rel_T:.1e}; "
    ...
    pos = np.array(
        [
            np.asarray(KeplerElements(a=a_o, e=e_o, M0=m).to_state(M_tot, G).position)
            for m in M0s
        ]
    )
    r_peri = float(
        jnp.linalg.norm(
            KeplerElements(a=a_o, e=e_o, M0=0.0).to_state(M_tot, G).position
        )
    )
    r_apo = float(
        jnp.linalg.norm(
            KeplerElements(a=a_o, e=e_o, M0=np.pi).to_state(M_tot, G).position
        )
    )
    e_peri = abs(r_peri - a_o * (1 - e_o))
    e_apo = abs(r_apo - a_o * (1 + e_o))
    p2 = e_peri < 1e-6 and e_apo < 1e-6
    print(
        f"  geometry: r_peri={r_peri:.4f} (a(1-e)={a_o * (1 - e_o)}), "
    ...
    E_num, E_an = [], []
    for ai in a_e:
        st = BinaryOrbitalState.from_semi_major_axis(
            m1=m1, m2=m2, a=ai, e=0.0, M_anom=0.0, G=G
        )
        ...
        E_num.append(KE + V)
        E_an.append(-G * m1 * m2 / (2 * ai))
    E_num, E_an = np.array(E_num), np.array(E_an)
    rel_E = np.max(np.abs(E_num - E_an) / np.abs(E_an))
    p3 = rel_E < 1e-3
    print(
        f"  energy: max rel(E, -G m1 m2/2a) = {rel_E:.1e}  -> {'PASS' if p3 else 'FAIL'}"
    )
    ...
    axA.set_xlabel(r"$a$ [AU]")
    axA.set_ylabel(r"period $T$ [yr]")
    ...
    axB.set_xlabel(r"$x$ [AU]")
    axB.set_ylabel(r"$y$ [AU]")
    ...
    axC.set_xlabel(r"$a$ [AU]")
    axC.set_ylabel(r"$-E$ [code units]")
...
def fig_moe_qdist(output_dir):
    ...
    ok = []
    for ax, (m1, col, tag) in zip(
        axes, [(1.0, OI["blue"], "(a)"), (10.0, OI["vermilion"], "(b)")]
    ):
        samp = np.asarray(
            qd.sample_given_primary(jax.random.PRNGKey(SEED), jnp.full(N, m1))
        )
        pdf = np.asarray(qd.pdf_given_primary(jnp.asarray(qgrid), m1))
        # analytic CDF (cumulative-trapezoid of the implemented pdf) for KS
        cdf = np.concatenate(
            [[0.0], np.cumsum(0.5 * (pdf[1:] + pdf[:-1]) * np.diff(qgrid))]
        )
        cdf /= cdf[-1]
        ss = np.sort(samp)
        ...
        ax.hist(
            ...
            edgecolor="white",
            linewidth=0.3,
            label=r"samples ($N{=}8\times10^4$)",
        )
        ax.plot(qgrid, pdf, "-", color=OI["black"], lw=1.8, label="implemented PDF")
        ax.set_title(
            rf"$M_1 = {m1:.0f}\,M_\odot$  (twins $={100 * twin:.1f}$" + r"$\%$)",
            fontsize=8.5,
        )
        ...
        print(
            f"  M1={m1:>4.0f} Msun: KS D={D:.4f} (D_crit={d_crit:.4f}), "
...
def fig_confidently_wrong(output_dir):
    ...
    an = np.array([r["an"] for r in rows])
    sn = np.array([r["sn"] for r in rows])
    aw = np.array([r["aw"] for r in rows])
    ...
    for r, ns_, as_ in zip(rows, naive_sig, aware_sig):
        print(
            f"  N={r['N']:>6d}  naive {r['an']:.3f}+-{r['sn']:.3f} "
            f"({ns_:.1f}sigma off)   aware {r['aw']:.3f}+-{r['sw']:.3f} "
            f"({as_:.1f}sigma)"
        )
    print(
        f"  f_bin={f_bin:.2f}; naive {naive_sig[-1]:.0f}sigma-wrong at N=1e5 "
    ...
    ax.annotate(
        "naive CI shrinks but\nstays off truth:\n"
        rf"{naive_sig[-1]:.0f}$\sigma$ wrong at $N{{=}}10^5$",
        xy=(N[-1], an[-1]),
        xytext=(2e3, an[-1] - 0.02),
        fontsize=6.8,
        color=OI["vermilion"],
...
def fig_bias_mechanism(output_dir):
    ...
    biases = []
    for fb in fbs:
        ms, _ = generate_system_masses(
            ALPHA_TRUE, 40_000, SEED + 7, f_model=ConstantBinaryFraction(fb)
        )
    ...
    print(
        f"  significance grows with N: {nsig[0]:.1f} -> {nsig[-1]:.0f}sigma  "
        f"({'PASS' if grows else 'FAIL'})"
    )
    print(
        f"  bias->0 at f_b=0: {biases[0]:+.3f} ({'PASS' if zero_ok else 'FAIL'}); "
...
def fig_gradient_validation(output_dir):
    ...
    specs = [
        (
            "a",
            r"$a$ [AU]",
            r"$\partial\,|v|^2/\partial a$",
            lambda a: jnp.sum(
                KeplerElements(a=a, e=0.3, M0=1.0).to_state(1.0, G).velocity ** 2
            ),
            np.linspace(0.8, 2.5, 11),
            1e-4,
        ),
        (
            "alpha_aware",
            r"$\alpha$",
            r"$\partial\,\mathrm{NLL}_{\rm aware}/\partial\alpha$",
            lambda al: -aware_loglike(ms, al),
            np.linspace(2.0, 2.6, 11),
            1e-3,
        ),
    ]
    fig, axes = plt.subplots(1, 2, figsize=(7.0, 3.0))
    ...
    for ax, (key, xlab, ylab, f, xs, h), tag in zip(axes, specs, "ab"):
        ...
        print(
            f"  d/d{key}: max rel err {np.max(rel):.2e}  "
            f"-> {'DIFFERENTIABLE' if np.max(rel) < 1e-3 else 'CHECK'}"
        )
    passed = worst < 1e-3
    print(
        f"  overall worst rel err {worst:.2e} (tol 1e-3)  "
...
def main():
    ...
    print("=" * 70)
    print(
        "  ALL BINARY VALIDATION FIGURES PASS"
        if all_ok
