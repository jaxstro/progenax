...
def fig_density(output_dir):
    ...
    print(
        f"  {'scale radius a [pc]':<26}{a_expected:>12.4f}{a:>12.4f}"
        f"{'PASS' if a_pass else 'FAIL':>8}"
    )
    print(
        f"  {'frac r<r_h':<26}{0.5:>12.3f}{frac_within_rh:>12.3f}"
        f"{'PASS' if rh_pass else 'FAIL':>8}"
    )
    print(
        f"  {'M(<a)/M':<26}{m_a_expected:>12.4f}{frac_within_a:>12.4f}"
        f"{'PASS' if ma_pass else 'FAIL':>8}"
    )
    print(
        f"  {'max |ECDF - CDF|':<26}{0.03:>12.3f}{max_cdf_dev:>12.4f}"
    ...
    axA.semilogy(
        xr,
        (1.0 + (np.asarray(r) / a) ** 2) ** -2.5,
        "-",
        color=OI["black"],
        lw=2.0,
        label=r"analytic $(1+r^2/a^2)^{-5/2}$",
    )
...
def fig_velocity_equilibrium(output_dir):
    ...
    print(
        f"  sigma_1d(r) vs GM/(6 sqrt(r^2+a^2)):  max rel = {max_disp_rel:.2%} "
    ...
    print(
        f"  virial Q=T/|V|: {Q:.4f} (expect 0.5+-0.05)  "
    ...
    axC.axhspan(
        0.45, 0.55, color=OI["green"], alpha=0.18, label=r"equilibrium $\pm0.05$"
    )
...
def fig_beta_distribution(output_dir):
    ...
    print(
        f"  <q^2>   expected {mean_exp:.4f}  measured {q2_mean:.4f}  "
        f"-> {'PASS' if mean_pass else 'FAIL'}"
    )
    print(
        f"  Var(q^2) expected {var_exp:.4f}  measured {q2_var:.4f}  "
    ...
    ax.plot(
        xg,
        np.asarray(_beta_pdf(jnp.asarray(xg), aB, bB)),
        "-",
        color=OI["vermilion"],
        lw=2.0,
        label=r"Beta$(3/2,\,9/2)$",
    )
    ax.axvline(mean_exp, color=OI["black"], ls=":", lw=1.0)
    ax.text(
        mean_exp + 0.02,
        ax.get_ylim()[1] * 0.6,
        rf"$\langle q^2\rangle={q2_mean:.3f}$",
        fontsize=8.5,
    )
...
def fig_gradient_validation(output_dir):
    ...
    specs = [
        (
            "r_h",
            r"$r_h$ [pc]",
            r"$\partial\,\log\rho(1\,{\rm pc}) / \partial r_h$",
            lambda rh: log_density_at(1.0, rh),
...
def fig_isotropy(output_dir):
    ...
    print(
        f"  cos(theta) ~ U(-1,1): reduced chi^2 = {chi2_cos:.2f} (expect ~1), "
        f"<cos> = {mean_cos:+.4f}  -> {'PASS' if cos_pass else 'FAIL'}"
    )
    print(
        f"  phi ~ U(0,2pi):       reduced chi^2 = {chi2_phi:.2f} (expect ~1)  "
        f"-> {'PASS' if phi_pass else 'FAIL'}"
    )
    print(
        f"  velocity isotropy:    max |<v_i^2>-mean|/mean = {vel_spread:.2%} "
    ...
    axB.axhline(
        1.0 / (2 * np.pi), color=OI["vermilion"], ls="--", lw=1.4, label=r"$U(0,2\pi)$"
    )
...
def main():
    ...
    print("=" * 70)
    print(
        "  ALL PLUMMER VALIDATION FIGURES PASS"
        if all_ok
