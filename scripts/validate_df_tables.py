...
def check_density_accuracy():
    ...
    exact = jax.vmap(lambda w, pp: _aniso_density_scalar(w, pp, jnp.asarray(G_PAR)))(
        W, p
    )
    central = float(
        _aniso_density_scalar(jnp.asarray(W_MAX), jnp.asarray(0.0), jnp.asarray(G_PAR))
    )
    out = {}
    for label, (n_W, n_p) in [
        ("512x96", (512, 96)),
        (f"{_TAB_N_W}x{_TAB_N_P}", (_TAB_N_W, _TAB_N_P)),
    ]:
        tab = AnisoDensityTable.build(
            W_max=W_MAX, p_max=P_MAX, g=G_PAR, n_W=n_W, n_p=n_p
        )
...
def check_gradients():
    ...
    ad = np.concatenate(
        [
            np.asarray(jax.grad(metric, 0)(w0, ra0)),
    ...
    ad_q = np.concatenate(
        [
            np.asarray(jax.grad(metric_q, 0)(w0, ra0)),
...
def main():
    ...
    checks.append(
        (
            "density max rel err, 512x96 grid <= 1e-5",
            rel_fine <= 1e-5,
            f"{rel_fine:.2e}",
        )
    )
    print(
        f"  (a) density max rel err: 512x96 grid {rel_fine:.2e} "
    ...
    for cfg, _, dpsi in solves:
        ...
        checks.append(
            (
                f"max|dpsi| <= 1e-4*W0 (W0={W0}, ra={cfg[2][0]}, xi_max={cfg[3]:.0f})",
                ok,
                f"{dpsi.max():.2e}",
            )
        )
        print(
            f"  (b) W0={W0} rescale={cfg[1]} ra={cfg[2]} xi_max={cfg[3]:.0f}: "
    ...
    checks.append(
        (
            "warm solve speedup >= 3x",
            sp_solve >= 3.0,
            f"{sp_solve:.1f}x ({t_q * 1e3:.0f} -> {t_t * 1e3:.0f} ms)",
        )
    )
    print(
        f"  (c) warm solve: quadrature {t_q * 1e3:.0f} ms, table {t_t * 1e3:.0f} ms "
        f"-> {sp_solve:.1f}x"
    )
    ...
    checks.append(
        (
            "aniso construction speedup >= 2x",
            sp_con >= 2.0,
            f"{sp_con:.1f}x ({tc_q * 1e3:.0f} -> {tc_t * 1e3:.0f} ms)",
        )
    )
    print(
        f"  (d) aniso construction: quadrature {tc_q * 1e3:.0f} ms, table "
    ...
    checks.append(
        (
            "AD = FD through table solve (w_j, r_a), rtol 1e-3",
            grad_ok,
            f"max rel diff {rel_g.max():.2e}",
        )
    )
    ...
    print(
        f"      cross-check table-AD vs quadrature-AD (FD-free): "
    ...
    axA.axhline(
        1e-5, color=OI["vermilion"], ls="--", lw=1.0, label="budget 1e-5 (512x96)"
    )
    axA.set_yscale("log")
    axA.set_xlabel(r"$W$")
    ...
    for (cfg, xi, dpsi), col in zip(solves, cols):
        ...
        axB.plot(
            xi / cfg[3],
            np.maximum(dpsi, 1e-12),
            color=col,
            lw=1.2,
            label=rf"$W_0={W0:.0f}$, $\hat r_a={cfg[2][0]:.0f}$",
        )
