...
def validate_two_component(N=800):
    ...
    model = MultiComponentCluster.from_components(
        alpha_j=jnp.array([0.5, 0.5]), w_j=jnp.array([0.7, 1.0]),
        m_j=jnp.array([1.0, 1.0]), W0=7.0, g=1.0, r_c=1.0)
    ic = model.sample_cluster(jax.random.PRNGKey(1), N, G=G)
    pos = np.asarray(ic.positions); cid = np.asarray(ic.component_id)
    ...
    print(f"  median r: cold (w=0.7) = {med_cold:.2f} pc  <  hot (w=1.0) = {med_hot:.2f} pc;  "
          f"theory Q_j = {np.round(Qj, 4)}  {'PASS' if ok else 'FAIL'}")
    fig, ax = plt.subplots(figsize=(7, 5))
    ax.hist(r[cid == 0], bins=30, alpha=0.6, density=True, label="cold (w=0.7, concentrated)")
    ax.hist(r[cid == 1], bins=30, alpha=0.6, density=True, label="hot (w=1.0, extended)")
    ax.set_xlabel("r (pc)"); ax.set_ylabel("density"); ax.legend()
    ax.set_title("MultiComponentCluster: two populations in one shared potential "
                 f"(Q_j = {np.round(Qj, 3)})")
