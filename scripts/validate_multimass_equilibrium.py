...
def _analytic_sigma1d(W_j, s_j):
    ...
    return float(
        s_j * jnp.sqrt(jnp.trapezoid(u**4 * E, u) / jnp.trapezoid(u**2 * E, u) / 3.0)
    )
...
def _half_mass_r(model, j):
    ...
    rho = limepy_density_hat(model.rescale_j[j] * psi, model.g) / limepy_density_hat(
        model.rescale_j[j] * model.W0, model.g
    )
    ...
    M = jnp.concatenate(
        [
            jnp.zeros(1),
            jnp.cumsum(0.5 * (integrand[1:] + integrand[:-1])) * (r[1] - r[0]),
        ]
    )
...
def main():
    ...
    model = MultiComponentCluster.from_mass_segregation(
        alpha_j=ALPHA_J, m_j=M_J, W0=W0, g=g, delta=0.5, r_c=1.0
    )
    ...
    for j, (mj, col, lab) in enumerate(
        [(M_J[0], LCOL, "light"), (M_J[1], HCOL, "heavy")]
    ):
        rho = limepy_density_hat(
            model.rescale_j[j] * psi_rr, model.g
        ) / limepy_density_hat(model.rescale_j[j] * model.W0, model.g)
        axA.plot(
            np.asarray(rr),
            np.asarray(rho / rho[0]),
            color=col,
            lw=1.8,
    ...
    axA.set_ylabel(r"$\rho_j(r)/\rho_j(0)$")
    ...
    for j, (mj, col, lab) in enumerate(
        ...
        axB.plot(
            rc_, sig_m, "o", color=col, ms=4, mfc="white", label=f"{lab} (sampled)"
        )
    axB.set_xlabel(r"$r/r_c$")
    axB.set_ylabel(r"$\sigma_{1\rm d,j}(r)$")
    ...
    for d in DELTAS:
        mdl = MultiComponentCluster.from_mass_segregation(
            alpha_j=ALPHA_J, m_j=M_J, W0=W0, g=g, delta=float(d), r_c=1.0
        )
    ...
    for d in DELTAS:
        ...
        Qlt_all.append(Qth[0])
        Qht_all.append(Qth[1])
        Ql_s, Qh_s, Qg_s = [], [], []
        for sd in SEEDS:
            ...
            masks = jnp.stack(
                [jnp.isclose(mm, float(mj)) for mj in M_J]
                + [jnp.ones_like(mm, dtype=bool)]
            )
            Qj = np.asarray(
                per_group_virial_ratio(p, v, mm, G=G, group_masks=masks, softening=0.0)
            )
            Ql_s.append(Qj[0])
            Qh_s.append(Qj[1])
            Qg_s.append(Qj[2])
        Ql_all.append(np.mean(Ql_s))
        Qh_all.append(np.mean(Qh_s))
        Qg_all.append(np.mean(Qg_s))
        Ql_se.append(np.std(Ql_s) / np.sqrt(len(Ql_s)))
        Qh_se.append(np.std(Qh_s) / np.sqrt(len(Qh_s)))
        print(
            f"  {d:.1f}    {np.mean(Ql_s):.3f}    {np.mean(Qh_s):.3f}    {np.mean(Qg_s):.3f}"
            f"   | {Qth[0]:.4f}, {Qth[1]:.4f}"
        )
    # theoretical (exact) -- flat at 0.5
    axD.plot(
        DELTAS, Qlt_all, "-", color=LCOL, lw=2.4, alpha=0.45, label="light (theory)"
    )
    axD.plot(
        DELTAS, Qht_all, "-", color=HCOL, lw=2.4, alpha=0.45, label="heavy (theory)"
    )
    ...
    axD.plot(
        DELTAS, Qg_all, "^", color="0.4", ms=4, ls="none", label="global (sampled)"
    )
    axD.axhline(0.5, color="0.6", ls="--", lw=1.0)
    axD.set_xlabel(r"equipartition $\delta$")
    axD.set_ylabel(r"per-group $Q_j = T_j/|W_j|$")
    ...
    Qj_ok = np.all(np.abs(np.array(Ql_all) - 0.5) < 0.06) and np.all(
        np.abs(np.array(Qh_all) - 0.5) < 0.10
    )  # heavy is rarer -> looser
    ...
    print(
        f"  saved seg_multimass_equilibrium.{{png,pdf}} -> {'PASS' if ok else 'FAIL'}"
    )
    print("=" * 70)
