...
def fig_density_king_limit(output_dir):
    ...
    rk = np.asarray(king.density(r)); rk = rk / rk[0]

    fig, ax = plt.subplots(figsize=(3.9, 3.5))
    ax.semilogy(np.asarray(r) / RC, rk, "-", color=OI["black"], lw=2.2,
                label="King (isotropic)", zorder=1)

    max_rel = 0.0
    for ra, col in [(RA_ISO, OI["sky"]), (20.0, OI["green"]), (RA_ANISO, OI["vermilion"])]:
        prof = MichieProfile.from_W0_rc(W0, RC, ra)
        rm = np.asarray(prof.density(r)); rm = rm / rm[0]
    ...
    print(f"  Michie(r_a->inf) vs King: max rel = {max_rel:.2e} (tol 1e-2)  "
          f"-> {'PASS' if iso_pass else 'FAIL'}")
    rt_a = float(MichieProfile.from_W0_rc(W0, RC, RA_ANISO).r_t)
    rt_i = float(king.r_t)
    ext_pass = rt_a > rt_i
    print(f"  r_t: aniso(r_a=8)={rt_a:.1f} > iso(King)={rt_i:.1f}  "
    ...
    ax.set_ylim(1e-4, 2)
    ax.legend(loc="upper right", title=r"$r_a\to\infty$ recovers King", title_fontsize=7.5)
...
def fig_beta_profile(output_dir):
    ...
    for lo, hi in edges:
        ...
        sr2 = float(np.mean(v_r[msk] ** 2)); st2 = float(np.mean(v_t[msk] ** 2))
        mids.append(rmid)
        beta_s.append(1.0 - st2 / (2.0 * sr2))
        W_mid = float(jnp.interp(rmid / RC, df.xi_grid, df.psi_grid, left=df.W0, right=0.0))
    ...
    oracle_curve = np.array([_beta_oracle(float(w), float(r) / RA_ANISO) for w, r in zip(Wc, rr)])
    ...
    print(f"  sampled beta vs DF oracle: max dev = {max_dev:.3f} (tol 0.05)  "
          f"-> {'PASS' if max_dev < 0.05 else 'FAIL'}")
    print(f"  beta below Osipkov-Merritt ceiling: {below_om}  "
          f"-> {'PASS' if below_om else 'FAIL'}")

    fig, ax = plt.subplots(figsize=(4.2, 3.5))
    ax.plot(rr / RA_ANISO, om_curve, ":", color="0.5", lw=1.5,
            label=r"Osipkov-Merritt $\frac{r^2}{r^2+r_a^2}$ (ceiling)")
    ax.plot(rr / RA_ANISO, oracle_curve, "-", color=OI["black"], lw=2.0,
            label="Michie-King DF oracle")
    ax.plot(mids / RA_ANISO, beta_s, "o", color=OI["vermilion"], ms=5, mec="white",
            mew=0.5, label=rf"sampled ($N={N_SAMPLES:,}$)")
    ...
    ax.text(0.04, 0.95, "isotropic core\n→ radial outward", transform=ax.transAxes,
            fontsize=8, va="top")
    fig.tight_layout(pad=0.4)
...
def fig_velocity_equilibrium(output_dir):
    ...
    Wv = jnp.maximum(jnp.interp(rv / df.r_c, df.xi_grid, df.psi_grid, left=df.W0, right=0.0), 0.0)
    ...
    Q = float(compute_kinetic_energy(vv, mv) / jnp.abs(compute_potential_energy(pv, mv, G=G)))
    bound_pass = bound_frac == 1.0
    q_pass = abs(Q - 0.5) < 0.05
    passed = bound_pass and q_pass
    print(f"  bound fraction: {bound_frac*100:.2f}%  -> {'PASS' if bound_pass else 'FAIL'}")
    print(f"  virial Q=T/|V|: {Q:.3f} (expect 0.5+-0.05)  -> {'PASS' if q_pass else 'FAIL'}")

    fig, (axA, axB, axC) = plt.subplots(1, 3, figsize=(7.2, 2.7))

    axA.errorbar(mids, sr, yerr=sr_e, fmt="o", color=OI["vermilion"], ms=5, capsize=2,
                 lw=1.0, label=r"$\sigma_r$ (radial)")
    ...
    axA.text(0.04, 0.1, r"$\sigma_r>\sigma_t$ outward", transform=axA.transAxes, fontsize=8)
...
def fig_king_limit_sweep(output_dir):
    ...
    max_rel = []
    for ra in ras:
        rm = np.asarray(MichieProfile.from_W0_rc(W0, RC, ra).density(r)); rm = rm / rm[0]
    ...
    passed = converges
    print(f"  max|d rho| at r_a/r_c=[5..1000]: "
          f"{[f'{x:.1e}' for x in max_rel]}")
    print(f"  converges (r_a=1000): {max_rel[-1]:.2e} < 1e-2  "
    ...
    ax.text(0.5, 0.06, r"$r_a\to\infty$: Michie $\to$ King", transform=ax.transAxes,
...
def fig_gradient_validation(output_dir):
    ...
    specs = [
        ...
        ("W0", r"$W_0$", r"$\partial\,\log\rho(2\,r_c) / \partial W_0$",
         lambda w: log_density_at(2.0, w, 1.0), np.linspace(4.0, 7.0, 9), 1e-3),
        ("r_c", r"$r_c$ [pc]", r"$\partial\,\log\rho(2\,{\rm pc}) / \partial r_c$",
         lambda rc: log_density_at(2.0, 7.0, rc), np.linspace(0.7, 1.6, 9), 1e-4),
        ("M", r"$M_{\rm tot}$ [$M_\odot$]", r"$\partial\,\sigma_0 / \partial M_{\rm tot}$",
         lambda M: vel_scale(M), np.linspace(300.0, 2000.0, 9), 1.0),
    ]
...
def main():
    ...
    print("=" * 70)
    print("  ALL MICHIE VALIDATION FIGURES PASS" if all_ok
