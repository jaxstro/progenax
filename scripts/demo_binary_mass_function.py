...
def _semimajor_axis(m1, m2, P_days):
    return np.asarray(
        period_to_semimajor_axis(jnp.asarray(P_days) * DAY, jnp.asarray(m1 + m2), G)
    )  # pc
...
def _blend_mass(m1, m2):
    L = zams_luminosity(jnp.asarray(m1), Z_MET) + zams_luminosity(
        jnp.asarray(m2), Z_MET
    )
...
def main():
    ...
    S_k, B_moe, B_ind, q_blend_moe, q_blend_ind = build_templates(
        jax.random.PRNGKey(SEED)
    )
    counts = build_data(jax.random.PRNGKey(SEED + 1))
    print(
        f"  blended-subset median q:  Moe = {q_blend_moe:.3f}   "
    ...
    print("\n  f_b recovery:")
    print(
        f"    Moe template (correct):     {fb_moe:.4f} +- {sig_moe:.4f}  "
        f"(pull {pull_moe:+.2f})"
    )
    print(
        f"    independent template (wrong): {fb_ind:.4f} +- {sig_ind:.4f}  "
    ...
    rows = [
        (
            "mechanism: blended q (Moe>ind)",
            "PASS" if mech_ok else "FAIL",
            "coupling",
            mech_ok,
        ),
        (
            "self-consistency (Moe@truth)",
        ...
        ("f_b BIAS (independent)", "PASS" if bias_ok else "FAIL", ">3 sigma", bias_ok),
    ]
    print("\n" + "-" * 78)
    ...
    print("=" * 78)
    print(
        "  BINARY MASS FUNCTION DEMO: ALL PASS"
        if all_ok
...
def make_figure(
    ...
    ax.plot(
        cen,
        N_SYS * F_B_TRUE * np.asarray(B_moe) / dlogm,
        "--",
        color=OI["vermilion"],
        label="binaries (blended+resolved)",
    )
    ax.set_xscale("log")
    ax.set_yscale("log")
    ...
    ax = axes[1]
    ax.bar([0, 1], [q_blend_moe, q_blend_ind], 0.55, color=[OI["vermilion"], OI["sky"]])
    ax.set_xticks([0, 1])
