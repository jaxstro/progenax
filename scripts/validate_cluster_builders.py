...
def table_virial_per_alias():
    ...
    builders = [
        ...
        ("eff", lambda k: build_eff_cluster(masses=m, a=1.0, gamma=5.0, r_t=15.0, key=k)),
        ("king", lambda k: build_king_cluster(masses=m, W0=7.0, r_c=1.0, key=k)),
        ("michie", lambda k: build_michie_cluster(masses=m, W0=7.0, r_c=1.0, r_a=8.0, key=k)),
        ("limepy", lambda k: build_limepy_cluster(masses=m, W0=5.0, g=1.0, r_c=1.0, key=k)),
    ]

    tol = 0.03
    print(f"  {'alias':<10}{'expected Q':>12}{'measured Q':>12}{'|delta|':>10}{'pass':>8}")
    all_pass = True
    for name, build in builders:
        ...
        print(f"  {name:<10}{0.5:>12.3f}{Q:>12.4f}{delta:>10.4f}"
...
def _density_recovery(profile, radii, r_lo, r_hi, n_bins, min_counts=50):
    ...
    rho_an = np.array([_shell_weighted_density(profile, bins[i], bins[i + 1])
...
def fig_density_recovery(output_dir):
    ...
    tol = 0.20
    all_pass = True
    print(f"  {'profile':<9}{'bins(>=50)':>11}{'max rel':>10}{'med rel':>10}{'pass':>8}")
    for label, prof, col, build, r_lo, r_hi, nb in cases:
        radii = np.asarray(jnp.linalg.norm(build(jax.random.PRNGKey(SEED)).positions, axis=1))
        ...
        ok = max_rel < tol
        all_pass = all_pass and ok
        print(f"  {label:<9}{int(np.sum(well)):>11d}{max_rel:>10.3f}"
        ...
        axA.loglog(centers[valid], rho_s[valid], "o",
                   color=col, ms=3.0, mec="none", alpha=0.6)
        axB.semilogx(centers[well], rel[well], "o", color=col, ms=3.0,
                     mec="none", alpha=0.7)

    axA.set_ylabel(r"$\rho(r) / \rho(r_{\rm lo})$")
    axA.set_ylim(1e-5, 5)
    axA.set_xlabel(r"$r$ [pc]")
    axA.legend(loc="lower left", title="lines: analytic; points: sampled",
               title_fontsize=7.5)
    panel_label(axA, "(a)", loc="upper right")

    axB.axhline(0.0, color="0.6", lw=0.7)
    axB.axhspan(0.0, tol, color=OI["green"], alpha=0.12)
    axB.axhline(tol, color=OI["vermilion"], ls="--", lw=1.0,
                label=rf"tol $={tol:.2f}$")
    ...
    print(f"  density shape recovered to < {tol:.0%} per well-sampled bin  ->  "
...
def fig_tidal_cut(output_dir):
    ...
    print(f"  {'max ghost mass (r>r_t)':<34}{0.0:>11.1f}{ghost_max_mass:>12.3e}"
          f"{'PASS' if ghosts_zero else 'FAIL':>8}")
    print(f"  {'min survivor mass (r<=r_t)':<34}{'>0':>11}{surv_min_mass:>12.3f}"
          f"{'PASS' if survivors_kept else 'FAIL':>8}")
    print(f"  {'leaked mass beyond r_t':<34}{0.0:>11.1f}{leaked:>12.3e}"
          f"{'PASS' if sharp else 'FAIL':>8}")
    print(f"  {'full build all massive':<34}{'True':>11}{str(full_intact):>12}"
          f"{'PASS' if full_intact else 'FAIL':>8}")
    surv_frac = float(np.mean(inside))
    print(f"  survivor fraction (r<=r_t={r_t}): {surv_frac:.3f}  "
    ...
    axA.plot(r_sorted, cum_full, "-", color=OI["blue"], lw=1.8,
             label="no tidal cut")
    axA.plot(r_sorted, cum_cut, "-", color=OI["vermilion"], lw=1.8,
    ...
    axB.plot(radii[sub], mass_cut[sub], "o", color=OI["vermilion"], ms=2.4,
             mec="none", alpha=0.4)
    axB.axvline(r_t, color="0.5", ls=":", lw=1.0)
    axB.text(r_t * 1.04, 0.5, r"$r_t$", color="0.4", fontsize=9)
    axB.text(0.55, 1.05, "survivors", color=OI["vermilion"], fontsize=8.5,
             transform=axB.get_yaxis_transform(), ha="left")
    ...
    print(f"  sharp cut + zero-mass ghosts beyond r_t  ->  "
...
def fig_rotation_Lz(output_dir):
    ...
    print(f"  fit intercept (base sample L_z) = {intercept:.3f} "
    ...
    ic_d = build_plummer_cluster(masses=jnp.ones(N_DIFF), r_h=1.0, key=key, rotation=spec)
    pos_d, vel_d = np.asarray(ic_d.positions), np.asarray(ic_d.velocities)
    R = np.sqrt(pos_d[:, 0] ** 2 + pos_d[:, 1] ** 2)
    vphi = (pos_d[:, 0] * vel_d[:, 1] - pos_d[:, 1] * vel_d[:, 0]) / np.maximum(R, 1e-10)
    ...
    diff_rel = float(np.max(np.abs(vmeas - van) / (van + 1e-9)))
    ...
    diff_rel_inner = float(np.max(np.abs(vmeas[inner] - van[inner]) / (van[inner] + 1e-9)))
    diff_pass = diff_rel_inner < 0.15
    print(f"  differential v_phi(R) overlay: max rel (R<=2.2) = {diff_rel_inner:.3f}  "
    ...
    axA.plot(omegas, Sigma_mR2 * omegas + intercept, "-", color=OI["black"], lw=1.6,
             label=rf"$\Sigma m R^2\,\omega + b$")
    axA.plot(omegas, Lz, "o", color=OI["blue"], ms=5, mfc="none", mew=1.3,
             label="measured $L_z$")
    ...
    axA.text(0.5, 0.06,
             ...
             transform=axA.transAxes, ha="center", va="bottom", fontsize=7.5,
    ...
    axB.plot(R_curve, v_curve, "-", color=OI["black"], lw=1.6,
             label=r"$v_{\rm peak}\frac{R}{R_p}e^{1-R/R_p}$")
    axB.errorbar(Rmid, vmeas, yerr=verr, fmt="s", color=OI["vermilion"], ms=5,
                 capsize=2.5, lw=1.0, label=r"binned $\langle v_\phi\rangle$")
...
def fig_anisotropy_beta(output_dir):
    ...
    ic = build_plummer_cluster(masses=m, r_h=1.0, key=key,
    ...
    print(f"  {'r [pc]':>8}{'beta_meas':>12}{'beta_analytic':>15}{'|dev|':>9}{'pass':>7}")
    for rm, bm, ba, ad in zip(rmid, beta_meas, beta_an, abs_dev):
        print(f"  {rm:>8.3f}{bm:>12.4f}{ba:>15.4f}{ad:>9.4f}"
              f"{'PASS' if ad < tol else 'FAIL':>7}")
    print(f"  max |beta_meas - beta_analytic| = {max_dev:.4f}  (tol {tol})  "
          f"->  {'PASS' if passed else 'FAIL'}")

    fig, ax = plt.subplots(figsize=(4.0, 3.4))
    r_curve = np.linspace(0.05, 3.0, 200)
    ax.plot(r_curve, r_curve ** 2 / (r_curve ** 2 + r_a ** 2), "-",
            color=OI["black"], lw=1.8,
            label=r"analytic $\beta(r)=r^2/(r^2+r_a^2)$")
    ax.errorbar(rmid, beta_meas, yerr=beta_err, fmt="o", color=OI["vermilion"],
    ...
    ax.text(0.04, 0.94, rf"max $|\Delta\beta|={max_dev:.3f}$", transform=ax.transAxes,
            fontsize=8, va="top",
...
def main():
    ...
    print("=" * 70)
    print("  ALL CLUSTER-BUILDER VALIDATION CHECKS PASS" if all_ok
