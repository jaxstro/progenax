...
def fig_jacobi_vs_l1(output_dir):
    ...
    print(
        f"  m/M_g=1e-7: |r_J - L1|/L1 = {realistic:.2e} (tol 1e-2)  "
        f"-> {'PASS' if real_ok else 'FAIL'}"
    )
    print(
        f"  rel err {rel[-1]:.1e} (m/M_g=1e-2) -> {rel[0]:.1e} (m/M_g=1e-9); "
    ...
    axA.set_xlabel(r"$m / M_{\rm g}$")
    axA.set_ylabel(r"tidal radius [pc]")
    ...
    axB.text(
        1.3e-7, 3e-3, "Galactic GC", fontsize=6.5, color="0.4", rotation=90, va="bottom"
    )
...
def fig_force_balance(output_dir):
    ...
    print(
        f"  self-gravity == 3 Omega^2 r at r = {cross:.2f} pc (r_J = {r_J:.2f})  "
    ...
    Phi = (
        -G * m / x - G * M_g / (R - x) - 0.5 * (G * (M_g + m) / R**3) * (x - x_com) ** 2
    )
    x_l1 = x[np.argmax(Phi)]
    l1_pass = abs(x_l1 - r_J) / r_J < 0.05
    print(
        f"  effective-potential max (L1) at x = {x_l1:.2f} pc (r_J = {r_J:.2f})  "
    ...
    axA.set_yscale("log")
    axA.set_xlabel(r"$r / r_J$")
    axA.set_ylabel(r"radial acceleration")
...
def fig_keplerian_vs_iso(output_dir):
    ...
    print(
        f"  r_J,iso / r_J,point = {ratio.mean():.4f} (expected (3/2)^1/3 = "
        f"{expected:.4f})  -> {'PASS' if ratio_ok else 'FAIL'}"
    )
    # Galactic GC anchor (R=8 kpc)
    iGC = np.argmin(np.abs(R - 8000.0))
    print(
        f"  at R=8 kpc: r_J,iso = {r_iso[iGC]:.1f} pc, r_J,point = {r_point[iGC]:.1f} pc"
    )
    ...
    ax.set_xlabel(r"orbital radius $R$ [kpc]")
    ax.set_ylabel(r"tidal radius $r_J$ [pc]")
    ax.legend(loc="upper left", fontsize=7)
    ax.text(
        0.96,
        0.06,
        rf"$r_{{\rm iso}}/r_{{\rm point}}=(3/2)^{{1/3}}={expected:.3f}$",
...
def fig_plummer_truncation(output_dir):
    ...
    bound = np.array(
        [
            float(
                jnp.sum(apply_tidal_truncation(pos, jnp.zeros((n, 3)), masses, rt)[2])
            )
            / n
            for rt in rts
        ]
    ...
    print(
        f"  bound-mass fraction vs analytic Plummer M(<r): max dev = {max_dev:.3f} "
    ...
    print(
        f"  r_J = {r_J:.1f} pc, fill factor r_h/r_J = {fill:.3f}; kept "
        f"{n_kept}/{n} ({100 * n_kept / n:.1f} pct)"
    )

    fig, (axA, axB) = plt.subplots(1, 2, figsize=(7.3, 3.1))
    axA.plot(
        rts / r_h, frac_an, "-", color=OI["black"], lw=1.9, label=r"analytic $M(<r)/M$"
    )
    axA.plot(
        rts[::2] / r_h,
        bound[::2],
        ...
        mfc="none",
        mew=1.0,
        label="truncated bound mass",
    )
    axA.set_xlabel(r"$r_t / r_h$")
    axA.set_ylabel(r"bound-mass fraction")
    ...
    axB.set_xlim(-1.5 * r_J, 1.5 * r_J)
    axB.set_ylim(-1.5 * r_J, 1.5 * r_J)
    axB.set_aspect("equal")
    axB.set_xlabel(r"$x$ [pc]")
    axB.set_ylabel(r"$y$ [pc]")
    axB.legend(loc="upper right", fontsize=6.5, markerscale=2)
    axB.text(
        0.04,
        0.05,
        rf"$r_t=r_J={r_J:.0f}$ pc",
        transform=axB.transAxes,
        fontsize=7,
        color=OI["vermilion"],
    )
...
def fig_gradient_validation(output_dir):
    ...
    drj_ad = np.array(
        [float(jax.grad(lambda mc: jacobi_radius(mc, M_g, R))(float(mc))) for mc in Mc]
    )
    drj_an = R / (3.0 * M_g) ** (1.0 / 3.0) * (1.0 / 3.0) * Mc ** (-2.0 / 3.0)
    rel_a = np.max(np.abs(drj_ad - drj_an) / np.abs(drj_an))
    a_ok = rel_a < 1e-6
    print(
        f"  d r_J/d M_c: AD vs analytic max rel err {rel_a:.2e} (tol 1e-6)  "
    ...
    dM_an = (
        n * 3.0 * a**2 * rts**2 / (rts**2 + a**2) ** 2.5
    )  # d/dr [N r^3/(r^2+a^2)^1.5]
    # mean-sense agreement (surrogate is a smoothed estimator; finite-N noise)
    rel_b = np.median(np.abs(dM_ad - dM_an) / (dM_an + 1e-9))
    b_ok = rel_b < 0.2
    print(
        f"  d(M_bound)/d r_t: surrogate vs analytic Plummer shell, median rel "
...
def main():
    ...
    print("=" * 70)
    print(
        "  ALL TIDAL VALIDATION FIGURES PASS"
        if all_ok
