...
def fig_density_slope(output_dir):
    ...
    slope = float(
        jnp.log(prof.density(jnp.array([r2]))[0] / prof.density(jnp.array([r1]))[0])
        / jnp.log(r2 / r1)
    )
    ...
    cdf_theory = np.asarray(
        jnp.interp(jnp.asarray(sorted_r), prof._r_grid, prof._cdf_grid)
    )
    ...
    print(
        f"  {'rho(a)':<28}{rho_a_exp:>12.4f}{rho_a:>12.4f}"
        f"{'PASS' if rho_a_pass else 'FAIL':>8}"
    )
    print(
        f"  {'outer slope d ln rho/d ln r':<28}{-gamma:>12.3f}{slope:>12.3f}"
        f"{'PASS' if slope_pass else 'FAIL':>8}"
    )
    print(
        f"  {'frac r<=r_t':<28}{1.0:>12.3f}{trunc_frac:>12.3f}"
        f"{'PASS' if trunc_pass else 'FAIL':>8}"
    )
    print(
        f"  {'max |ECDF-CDF|':<28}{0.02:>12.3f}{max_cdf_dev:>12.4f}"
    ...
    axA.loglog(
        xr,
        (1.0 + (np.asarray(r) / a) ** 2) ** (-gamma / 2.0),
        "-",
        color=OI["black"],
        lw=2.0,
        label=r"analytic $(1+r^2/a^2)^{-\gamma/2}$",
    )
    ...
    axA.loglog(
        xg,
        rho_an_c[0] * (centers[0] / a) ** gamma * xg ** (-gamma) * 0.4,
        ":",
        color=OI["vermilion"],
        lw=1.3,
        label=rf"$r^{{-\gamma}},\ \gamma={gamma:.0f}$",
    )
    ...
    axB.plot(
        np.asarray(prof._r_grid) / a,
        np.asarray(prof._cdf_grid),
        "-",
        color=OI["black"],
        lw=2.0,
        label="analytic CDF",
    ...
    axB.text(
        0.04,
        0.94,
        rf"max $|\Delta\mathrm{{CDF}}|={max_cdf_dev:.1e}$",
...
def fig_gamma_family(output_dir):
    ...
    print(
        f"  gamma=5 vs Plummer (a=1): max rel = {max_rel_pl:.2e}  "
    ...
    for g in gammas:
        rad = jnp.linalg.norm(
            EFFProfile(a=a, gamma=g, r_t=r_t).sample_positions(
                jnp.ones(20_000), jax.random.PRNGKey(7)
            ),
            axis=1,
        )
    ...
    print(
        f"  median radius vs gamma=[2,3,4,5]: {[round(m, 2) for m in med]}  "
    ...
    ax.set_ylim(1e-4, 2)
    ax.legend(
        loc="upper right", title=r"$\gamma=5$ overlies Plummer", title_fontsize=7.5
    )
...
def _virial_Q(gamma, r_t, n=N_VIRIAL, seed=0):
    ...
    Q = float(
        compute_kinetic_energy(vel, m) / jnp.abs(compute_potential_energy(pos, m, G=G))
    )
...
def fig_velocity_equilibrium(output_dir):
    ...
    print(
        f"  velocity isotropy: max|<v_i^2>-mean|/mean = {iso_spread:.2%} (tol 10%)  "
    ...
    print(
        f"  virial Q (gamma=5, mild trunc): {Q5:.3f} (expect 0.5+-0.05)  "
        f"-> {'PASS' if q5_pass else 'FAIL'}"
    )
    print(
        f"  virial Q (gamma=3, sharp trunc): {Q3:.3f}  (caveat: sub-virial, see test)"
    )
    ...
    axC.plot(
        [0], [Q3], "D", color=OI["orange"], ms=7, zorder=5, label=r"$\gamma=3$ (sharp)"
    )
    axC.text(
        0.5,
        0.9,
...
def fig_gradient_validation(output_dir):
    ...
    specs = [
        (
            "a",
            r"$a$ [pc]",
            r"$\partial\,\log\rho(2a) / \partial a$",
            lambda a: log_density_at(2.0, a, 4.0),
        ...
        (
            "gamma",
            r"$\gamma$",
            r"$\partial\,\rho(2a) / \partial \gamma$",
            lambda g: density_at(2.0, 1.0, g),
        ...
        (
            "M",
            r"$M_{\rm tot}$ [$M_\odot$]",
            r"$\partial\,\sqrt{\kappa} / \partial M_{\rm tot}$",
    ...
    for ax, (key, xlab, ylab, loss, xs, h), tag in zip(axes, specs, "abc"):
        ...
        print(
            f"  d(loss)/d{key:5}: max rel err {np.max(rel):.2e}  "
...
def fig_eddington_f(output_dir):
    ...
    for gamma, col in [(3.0, OI["green"]), (4.0, OI["orange"]), (5.0, OI["vermilion"])]:
        ...
        print(
            f"  gamma={gamma:.0f}: min f={f.min():.2e} (>=0:{nonneg}), "
    ...
    ax.legend(
        loc="upper left", title=r"$f(\mathcal{E})\geq 0$, increasing", title_fontsize=8
    )
    fig.tight_layout(pad=0.4)
...
def main():
    ...
    print("=" * 70)
    print(
        "  ALL EFF VALIDATION FIGURES PASS"
        if all_ok
