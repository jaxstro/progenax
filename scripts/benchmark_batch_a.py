...
def run_case(kind: str, workload: str, n: int, src: str, extra) -> None:
    ...
    if workload == "pe":
        ...
        def call():
            df.sample_velocities(pos, masses, jax.random.PRNGKey(1),
                                 G=STELLAR.G).block_until_ready()
    ...
    if kind == "mem":
        ...
        out = {"wall_s_median": round(statistics.median(walls), 4),
...
def _ensure_worktree(worktree: Path) -> None:
    if (worktree / "src" / "progenax").is_dir():
        return
    subprocess.run(["git", "worktree", "add", "--detach", str(worktree), PRE_REF],
...
def _label(workload, n):
    name = ("potential energy" if workload == "pe"
            else "aniso LIMEPY DF sampling")
    return f"{name} N={n:,}"
...
def make_figures(results: dict) -> None:
    ...
    pe_ns = [2_000, 8_000]
    axa.plot(pe_ns, [r[f"time_pe_{n}_before"]["wall_s_median"] for n in pe_ns],
             "o-", color=OI["vermilion"], label=f"pre-batch ({PRE_REF})")
    pe_ns_after = [2_000, 8_000, 20_000]
    axa.plot(pe_ns_after,
             [r[f"time_pe_{n}_after"]["wall_s_median"] for n in pe_ns_after],
             "s-", color=OI["blue"], label="Batch A (blocked)")
    axa.annotate(f"N=20,000 pre-batch:\nmemory-infeasible\n"
                 f"({r['mem_pe_20000_before']['peak_rss_gb']:.1f} GB peak RSS)",
                 xy=(0.97, 0.05), xycoords="axes fraction", ha="right",
                 fontsize=7.5, color=OI["vermilion"])
    axa.set_xscale("log")
    axa.set_yscale("log")
    ...
    df_ns = [2_000, 10_000]
    axb.plot(df_ns, [r[f"time_df_{n}_before"]["wall_s_median"] for n in df_ns],
             "o-", color=OI["vermilion"], label="pre-batch (quadrature)")
    df_ns_after = [2_000, 10_000, 20_000]
    axb.plot(df_ns_after,
             [r[f"time_df_{n}_after"]["wall_s_median"] for n in df_ns_after],
             "s-", color=OI["blue"], label="Batch A (table)")
    axb.plot([10_000], [r["time_df_10000_quad"]["wall_s_median"]], "D",
...
def main() -> int:
    ...
    parser.add_argument("--case", help="run a single case in-process "
                                       "(subprocess re-entry)")
    ...
    gate_ok = ratio >= 5.0
    print("-" * 76)
    print(f"  memory gate: PE N=8k before/after = {ratio:.1f}x "
    ...
    payload = {
        "meta": {
            ...
            "before_ref": PRE_REF, "time_reps": TIME_REPS,
        },
        "results": results,
