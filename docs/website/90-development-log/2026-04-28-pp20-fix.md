...
description: Development-log record of the 2026-04-28 fix to the PP20 magnification factor ζ(p); the code now lives in gravoturb.theory.dense_gas_sfr after the 2026-06 clean-room rewrite.
---
...
```{admonition} 2026-06 update — code moved to gravoturb
:class: note
...
subsystem was rebuilt as the experimental, repo-only **`gravoturb`**
package; the corrected ζ(p) now lives in `gravoturb.theory.dense_gas_sfr.magnification_factor`
and is re-verified by the committed acceptance suite (AC3 — see
`src/experimental/gravoturb/VALIDATION_SUMMARY.md`). Paths below are the
