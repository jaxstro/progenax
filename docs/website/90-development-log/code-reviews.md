...
| **M7** tooling | ◑ Partial | `47568b4` | CI + pytest markers + coverage added; `uv.lock` deferred |
...
**Deferred / ticketed** (science sub-efforts or refactors, out of hardening scope):

- **M3** — BM19 *production-field* (β≈4) dense-tail validation → tracked in
  `docs/notes/2026-06-02-bm19-field-tail-ticket.md`.
- **M9** — runtime warning on the stub FDF calibration (`fdf_calibration.py` v1,
  χ~D) and surfacing its `version` — not yet added.
- Splitting the three oversize units (file-length refactor) — not done.
