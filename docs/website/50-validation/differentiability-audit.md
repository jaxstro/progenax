...
classifies it. Test gate: `tests/validation/grad_audit/` (driven by
...
:class: important
Across **99** entry-point × parameter cases (**98 clean**, **1 pinned known-limitation**, **0
unresolved hazards** — live counts from the regenerated
`validation/data/grad_audit_results.json`; three formerly-pinned $\alpha=1$ edges
became *clean* when audit S4 fixed them outright), the audit found **exactly two** gradient bugs,
...
Case (99 total), generated by `scripts/audit_gradients.py --plots` directly from
...
One-glance summary of the 99-case release gradient-gate, by protected direction (params
$\rightarrow$ IC vs params $\rightarrow$ summary) and status. **73 clean** in the IC
direction, **25 clean / 1 known-limitation** in the summary direction — **98 clean,
