...
def test_envelope_reproduces_plummer_profile_when_no_turbulence():
    ...
    np.testing.assert_allclose(rho, np.asarray(prof.density(jnp.asarray(r))), rtol=1e-10)
...
def test_envelope_is_separable():
    ...
    np.testing.assert_allclose(np.asarray(s_total - s_env), np.asarray(s_turb), atol=1e-8)
...
def test_envelope_differentiable_in_rh():
    ...
    def total(r_h):
        s_total = apply_spherical_envelope(s_turb, PlummerProfile(r_h=r_h), box_size=BOX)
