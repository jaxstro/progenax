...
def test_diagnostics_on_iid_gaussian_chains():
    ...
    diag = compute_hmc_diagnostics(
        positions,
        divergences=np.zeros((4, 1000), bool),
        energy=rng.standard_normal((4, 1000)),
        tree_depth=np.full((4, 1000), 5),
        max_tree_depth=10,
...
def test_diagnostics_saturation_and_low_bfmi_each_fail():
    ...
    low = compute_hmc_diagnostics(
        positions, energy=walk_energy, param_names=["M", "alpha"]
    )
    assert low["bfmi"] < 0.3
