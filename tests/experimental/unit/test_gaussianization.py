...
``s = T(g) = log_density_icdf(Phi(g)) - log<e^s>`` carries the BM19 marginal onto a unit
...
def test_hermite_coefficients_lognormal_limit_variance():
    """Large alpha (tail negligible): sum_{n>=1} c_n^2/n! ~ sigma_s^2 = Var[s]."""
    from gravoturb.theory.density_pdf import sigma_s_squared
    from gravoturb.theory.log_correlations import log_density_hermite_coefficients

    mach, b, alpha = 5.0, 0.4, 6.0
    sig2 = float(sigma_s_squared(mach, b))
    c = np.asarray(log_density_hermite_coefficients(mach, b, alpha, n_max=10))
    fact = np.array([factorial(k) for k in range(len(c))], dtype=float)
    var_series = float(np.sum(c[1:] ** 2 / fact[1:]))
    assert var_series == pytest.approx(sig2, rel=0.05)


def test_hermite_coefficients_differentiable():
    """c_n(mach,b,alpha) differentiable: grad of c_2 wrt mach is finite."""
    from gravoturb.theory.log_correlations import log_density_hermite_coefficients

    def c2(mach):
        return log_density_hermite_coefficients(mach, 0.4, 2.0, n_max=4)[2]
...
def test_gaussianized_xi_zero_at_rho0_and_variance_at_rho1():
    ...
    c = log_density_hermite_coefficients(5.0, 0.4, 2.0, n_max=12)
    fact = np.array([factorial(k) for k in range(len(c))], dtype=float)
...
def test_gaussianized_xi_differentiable_in_params():
    ...
    def xi_at(mach):
        c = log_density_hermite_coefficients(mach, 0.4, 2.0, n_max=8)
