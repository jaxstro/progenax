...
def test_volume_pdf_normalized(mach, b, alpha):
    ...
    p = np.asarray(log_density_pdf(s, mach, b, alpha))
...
def test_icdf_analytic_roundtrip(mach, b, alpha):
    ...
    from gravoturb.theory.density_cdf import log_density_icdf_analytic, build_cdf_table

    u = np.array([1e-6, 0.01, 0.2, 0.5, 0.8, 0.99, 1 - 1e-6, 1 - 5e-7])
    s = np.asarray(log_density_icdf_analytic(u, mach, b, alpha))
    # recover u via the analytic mass-independent volume CDF table (fine interp)
    s_grid, cdf = build_cdf_table(mach, b, alpha, n_nodes=200_000)
...
def test_icdf_analytic_differentiable():
    ...
    import jax
    from gravoturb.theory.density_cdf import log_density_icdf_analytic

    for u in (0.3, 0.95):
        g = float(
            jax.grad(
                lambda a: jnp.sum(log_density_icdf_analytic(jnp.array([u]), 6.0, 0.4, a))
            )(1.8)
        )
...
def test_mass_cdf_matches_numeric(mach, b, alpha):
    ...
    from gravoturb.theory.density_cdf import mass_cdf, log_density_pdf
    ...
    m_analytic = np.asarray(mass_cdf(grid, mach, b, alpha))
...
def test_mass_cdf_at_transition_is_one_minus_f_dense(mach, b, alpha):
    ...
    from gravoturb.theory.density_cdf import mass_cdf

    s_t = transition_density(alpha, sigma_s_squared(mach, b))
    f_dense = float(dense_mass_fraction(mach, b, alpha))
    assert 1.0 - float(mass_cdf(s_t, mach, b, alpha)) == pytest.approx(
        f_dense, rel=1e-4
    )
...
def test_mean_density_matches_numeric(mach, b, alpha):
    """mean_density = ⟨ρ/ρ_0⟩ = ∫ e^s p ds ≥ 1 (power-law tail adds mass)."""
    from gravoturb.theory.density_cdf import mean_density, log_density_pdf

    s = np.linspace(-30.0, 90.0, 2_000_000)
    numeric = np.trapezoid(
        np.exp(s) * np.asarray(log_density_pdf(s, mach, b, alpha)), s
    )
    val = float(mean_density(mach, b, alpha))
    assert val >= 1.0
...
def test_volume_tail_fraction_matches_integral(mach, b, alpha):
    """volume_tail_fraction = ∫_{s_t}^∞ p(s) ds (volume fraction above s_t)."""
    from gravoturb.theory.density_pdf import sigma_s_squared, transition_density
    from gravoturb.theory.density_cdf import log_density_pdf, volume_tail_fraction

    s_t = float(transition_density(alpha, sigma_s_squared(mach, b)))
    s = np.linspace(s_t, s_t + 200.0, 2_000_000)
    integral = np.trapezoid(np.asarray(log_density_pdf(s, mach, b, alpha)), s)
    frac = float(volume_tail_fraction(mach, b, alpha))
    assert 0.0 < frac < 1.0
...
def test_volume_pdf_continuous_at_st(mach, b, alpha):
    ...
    s_t = float(transition_density(alpha, sigma_s_squared(mach, b)))
    lo = float(log_density_pdf(s_t - 1e-6, mach, b, alpha))
    hi = float(log_density_pdf(s_t + 1e-6, mach, b, alpha))
    assert lo == pytest.approx(hi, rel=1e-4)


@pytest.mark.parametrize("mach,b,alpha", PARAMS)
def test_cdf_monotone_and_bounds(mach, b, alpha):
    from gravoturb.theory.density_cdf import build_cdf_table

    s_grid, cdf = build_cdf_table(mach, b, alpha)
...
def test_icdf_roundtrip(mach, b, alpha):
    ...
    import jax.numpy as jnp
    from gravoturb.theory.density_cdf import log_density_icdf, build_cdf_table

    s_grid, cdf = build_cdf_table(mach, b, alpha)
    u = jnp.linspace(0.02, 0.98, 50)
    s = log_density_icdf(u, mach, b, alpha)
...
def test_icdf_monotone_in_u():
    ...
    s = np.asarray(log_density_icdf(jnp.linspace(0.05, 0.95, 100), 5.0, 0.4, 2.0))
    assert np.all(np.diff(s) > 0)


def test_icdf_differentiable_in_params():
    """log_density_icdf is differentiable in alpha (grad-safe table) - enables P2 copula grads."""
    import jax.numpy as jnp
    from gravoturb.theory.density_cdf import log_density_icdf

    def mean_s(alpha):
        u = jnp.linspace(0.1, 0.9, 64)
        return jnp.mean(log_density_icdf(u, 5.0, 0.4, alpha))
