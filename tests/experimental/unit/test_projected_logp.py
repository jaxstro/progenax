...
def test_uses_logdensity_chain_steeper_than_density():
    ...
    T = jnp.ones(NB)
    mu = predict_logp_bandpowers(SHAPE, 3.0, 8.0, 0.4, 2.5, 32.0, K_EDGES, T, n_max=8, n_quad=64)
    _kc, P_rho, _nm = angular_bandpowers_2d_limber(SHAPE, 3.0, 8.0, 0.4, 2.5, 32.0, K_EDGES, 8, 64)
...
def test_transfer_scales_per_bin():
    ...
    base = predict_logp_bandpowers(SHAPE, 3.0, 8.0, 0.4, 2.5, 32.0, K_EDGES, jnp.ones(NB), 8, 64)
    scaled = predict_logp_bandpowers(SHAPE, 3.0, 8.0, 0.4, 2.5, 32.0, K_EDGES, T, 8, 64)
    np.testing.assert_allclose(np.asarray(scaled), np.asarray(base) * np.asarray(T), rtol=1e-10)
...
def test_differentiable_in_beta():
    ...
    def total(beta):
        return jnp.sum(predict_logp_bandpowers(SHAPE, beta, 8.0, 0.4, 2.5, 32.0, K_EDGES, T, 8, 64))
...
def test_analytic_emulator_matches_direct_model():
    ...
    table = precompute_a_s_table(SHAPE, 8.0, 0.4, 2.5, 32.0, K_EDGES, beta_nodes, n_max=8, n_quad=64)
    ...
    mu_dir = predict_logp_bandpowers(SHAPE, beta_off, 8.0, 0.4, 2.5, 32.0, K_EDGES, T, 8, 64)
...
def test_analytic_emulator_differentiable():
    ...
    from gravoturb_fdf.inference.projected_logp import interp_logp_bandpowers, precompute_a_s_table

    beta_nodes = jnp.linspace(2.0, 11.0 / 3.0, 96)
    table = precompute_a_s_table(SHAPE, 8.0, 0.4, 2.5, 32.0, K_EDGES, beta_nodes, n_max=8, n_quad=64)
    T = jnp.ones(NB)
    g = float(jax.grad(lambda b: jnp.sum(interp_logp_bandpowers(b, beta_nodes, table, T)))(2.917))
    assert np.isfinite(g) and g != 0.0
...
def test_loglike_stationary_and_peaked_at_truth():
    ...
    prec = jnp.eye(NB)
    beta_true = 3.0
    data = predict_logp_bandpowers(SHAPE, beta_true, 8.0, 0.4, 2.5, 32.0, K_EDGES, T, 8, 64)

    def ll(beta):
        return logp_loglike(data, beta, 8.0, 0.4, 2.5, 32.0, SHAPE, K_EDGES, T, prec, 8, 64)
...
def test_loglike_differentiable_off_truth():
    ...
    g = float(jax.grad(lambda b: logp_loglike(data, b, 8.0, 0.4, 2.5, 32.0, SHAPE, K_EDGES, T, prec, 8, 64))(2.5))
    assert np.isfinite(g) and g != 0.0
...
def test_calibrate_transfer_makes_model_match_fiducial_mean():
    ...
    np.testing.assert_allclose(np.asarray(a_s_fid) * np.asarray(T), rows.mean(axis=0), rtol=1e-10)
