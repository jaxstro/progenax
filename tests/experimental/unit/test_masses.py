...
def test_lambda1_perfect_density_mass_correlation():
    ...
    assigned = correlated_mass_assignment(
        masses, dens, lambda_corr=1.0, key=jax.random.PRNGKey(1)
    )
...
def test_lambda0_no_correlation():
    ...
    for s in range(20):
        a = correlated_mass_assignment(
            masses, dens, lambda_corr=0.0, key=jax.random.PRNGKey(s)
        )
        rhos.append(spearmanr(np.asarray(a), np.asarray(dens))[0])
    assert abs(float(np.mean(rhos))) < 0.1, (
        f"λ=0 mean ρ={np.mean(rhos):.3f}, expected ≈0"
    )
...
def test_correlation_monotonic_in_lambda():
                    ...
                    spearmanr(
                        np.asarray(
                            correlated_mass_assignment(
                                masses, dens, lambda_corr=lam, key=jax.random.PRNGKey(s)
                            )
                        ),
                        np.asarray(dens),
                    )[0]
    ...
    assert all(rhos[i] <= rhos[i + 1] + 0.05 for i in range(len(rhos) - 1)), (
        f"not monotonic: {rhos}"
    )
...
def test_mass_conserving_permutation():
    ...
    for lam in (0.0, 0.5, 1.0):
        a = correlated_mass_assignment(
            masses, dens, lambda_corr=lam, key=jax.random.PRNGKey(3)
        )
        assert jnp.allclose(jnp.sort(a), jnp.sort(masses)), (
            f"masses not conserved at λ={lam}"
        )
...
def test_reproducible_with_key():
    ...
    a = correlated_mass_assignment(
        masses, dens, lambda_corr=0.6, key=jax.random.PRNGKey(7)
    )
    b = correlated_mass_assignment(
        masses, dens, lambda_corr=0.6, key=jax.random.PRNGKey(7)
    )
