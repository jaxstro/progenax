...
def test_blocks_from_eps_matches_per_star_blocks():
    ...
    assert jnp.all(
        jnp.diagonal(Mb_noisy, axis1=-2, axis2=-1)
...
def test_depth_criterion_grad_AD_vs_FD():
    ...
    loss = lambda zz, uu: oed.c_criterion(
        oed_depth.depth_fisher_u(zz, uu, 4000.0), target=1
    )
    ...
    for i in (0, 17, 31):
        ...
        zm = z.at[i].add(-eps)
        assert jnp.allclose(
            g_ad[0][i], (loss(zp, u) - loss(zm, u)) / (2 * eps), rtol=1e-4, atol=1e-8
        )
...
def test_joint_optimizer_beats_fixed_depth():
    ...
    res = oed_depth.optimize_depth_design(
        target=1, N_total=400.0, key=jax.random.PRNGKey(1), n_starts=6, n_steps=400
    )
    # the jointly-optimised design beats a shallow and a very-deep fixed depth
    assert res.criterion < oed_depth.crit_at_fixed_depth(
        m_lim=10.0, target=1, N_total=400.0
    )
    assert res.criterion < oed_depth.crit_at_fixed_depth(
        m_lim=16.0, target=1, N_total=400.0
    )
...
def test_sigma_M_has_interior_optimum_in_depth():
    ...
    sigM = oed_depth.sigma_M_vs_depth(
        m_grid, target=1, N_total=400.0, n_starts=3, n_steps=250
    )
...
def test_depth_fisher_calibration_is_validated_and_bounded():
    ...
    n_draws = 64
    realized, predicted = oed_depth.calibrate_depth_fisher(
        res.z, res.m_lim, 400.0, n_draws, key
    )
