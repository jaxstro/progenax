...
def test_apparent_mag_and_distance_modulus():
    ...
    assert jnp.allclose(sel.apparent_mag(jnp.array(1.0), d_pc=10.0, Z=0.02), 5.13, atol=0.02)
    # (3) distance-modulus identity (DM=0 at 10 pc) + farther -> fainter (larger m_app)
    assert jnp.allclose(sel.apparent_mag(jnp.array(1.0), 10.0), sel.abs_bol_mag(jnp.array(1.0)))
