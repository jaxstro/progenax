...
class TestMultiMassCoreDelta0:
    ...
    def test_delta0_recovers_single_mass_potential(self, g):
        ...
        np.testing.assert_allclose(
            np.asarray(psi_m), np.asarray(psi_s), rtol=1e-9, atol=1e-9
        )
    ...
    def test_delta0_components_share_single_mass_density(self):
        ...
        for j in range(2):
            np.testing.assert_allclose(
                np.asarray(rho_j[j]), np.asarray(single), rtol=1e-6, atol=1e-8
            )
...
def _half_mass_xi(xi, rho):
    ...
    dxi = xi[1] - xi[0]
    M = jnp.concatenate(
        [jnp.zeros(1), jnp.cumsum(0.5 * (integrand[1:] + integrand[:-1])) * dxi]
    )
...
class TestMultiMassSegregation:
    ...
    def test_heavy_component_is_more_centrally_concentrated(self):
        ...
        assert xh_heavy < xh_light, (
            f"heavy r_h={xh_heavy:.2f} not < light r_h={xh_light:.2f}"
        )
    ...
    def test_segregation_strength_increases_with_delta(self):
        ...
        assert abs(ratios[0] - 1.0) < 1e-3, (
            f"delta=0 should give no segregation: {ratios[0]:.3f}"
        )
        assert np.all(np.diff(ratios) > 0), (
            f"segregation not monotonic in delta: {ratios}"
        )
    ...
    def test_differentiable_in_W0_g_delta_alpha(self):
        ...
        assert (
            jnp.isfinite(dd) and jnp.abs(dd) > 0
        )  # delta genuinely moves the solution
...
class TestEigenvalueSolve:
    ...
    def _kroupa_like_components(self, n=4):
        ...
        M_j = np.array(
            [(e1**-0.3 - e0**-0.3) / -0.3 for e0, e1 in zip(m_edges[:-1], m_edges[1:])]
        )
    ...
    def test_realized_masses_match_targets(self):
        ...
        assert float(residual) < 1e-3, (
            f"eigenvalue residual {float(residual):.2e} too large"
        )
        # independent check of the realized mass fractions
        xi, psi, _, rho_j = solve_multimass_limepy(
            alpha_j, m_j, 7.0, 1.0, 0.5, 300.0, 2000
        )
    ...
    def test_alpha_normalized_and_positive(self):
        ...
        alpha_j, _ = find_alpha_for_masses(
            m_j, M_j, W0=7.0, g=1.5, delta=0.4, n_iter=30, xi_max=300.0, n_points=2000
        )
    ...
    def test_anisotropic_realized_masses_match_targets(self):
        ...
        assert float(residual) < 2e-3, (
            f"aniso eigenvalue residual {float(residual):.2e}"
        )
    ...
    def test_differentiable_in_targets_and_delta(self):
        ...
        def metric_M(M):
            a, _ = find_alpha_for_masses(
                m_j, M, 7.0, 1.0, 0.4, n_iter=15, xi_max=300.0, n_points=1500
            )
            return jnp.sum(a**2)

        def metric_d(delta):
            a, _ = find_alpha_for_masses(
                m_j, M_j, 7.0, 1.0, delta, n_iter=15, xi_max=300.0, n_points=1500
            )
            return jnp.sum(a**2)
...
class TestMultiMassAnisotropic:
    ...
    def test_isotropic_corner_ra_none(self):
        ...
        xi_i, psi_i, _, _ = solve_multimass_limepy(
            alpha, m_j, 7.0, 1.0, 0.4, 300.0, 2000
        )
        xi_a, psi_a, _, _ = solve_multimass_limepy(
            alpha, m_j, 7.0, 1.0, 0.4, 300.0, 2000, ra_hat=None
        )
        np.testing.assert_allclose(
            np.asarray(psi_a), np.asarray(psi_i), rtol=1e-9, atol=1e-9
        )
    ...
    def test_single_component_recovers_single_mass_anisotropic(self):
        ...
        xi_s, psi_s, _ = solve_limepy_profile(
            7.0, g=1.0, ra_hat=8.0, xi_max=800.0, n_points=3000
        )
        ...
        inside = psi_s > 1e-3
        np.testing.assert_allclose(
            np.asarray(jnp.interp(xi_s, xi_m, psi_m))[inside],
            np.asarray(psi_s)[inside],
            rtol=2e-3,
            atol=2e-3,
        )
    ...
    def test_anisotropic_multimass_differentiable_in_eta_ra_delta(self):
        ...
        def metric(ra_hat, eta, delta):
            xi, psi, _, _ = solve_multimass_limepy(
                alpha, m_j, 7.0, 1.0, delta, 800.0, 2000, ra_hat=ra_hat, eta=eta
            )
...
class TestDirectPerComponentScales:
    ...
    def test_equal_rescale_recovers_single_component(self):
        ...
        np.testing.assert_allclose(
            np.asarray(psi), np.asarray(psi_s), rtol=1e-9, atol=1e-9
        )
    ...
    def test_mass_wrapper_matches_direct_core(self):
        ...
        _, psi_a, _, rho_a = solve_multimass_limepy(
            alpha, m_j, 7.0, 1.0, delta, 300.0, 2000
        )
        _, psi_b, _, rho_b = solve_multicomponent_limepy(
            alpha, rescale, W0=7.0, g=1.0, xi_max=300.0, n_points=2000
        )
        np.testing.assert_allclose(
            np.asarray(psi_b), np.asarray(psi_a), rtol=1e-11, atol=1e-11
        )
        np.testing.assert_allclose(
            np.asarray(rho_b), np.asarray(rho_a), rtol=1e-9, atol=1e-9
        )
    ...
    def test_all_inf_ra_hat_j_returns_finite_psi(self):
        ...
        xi, psi, _, rho_j = solve_multicomponent_limepy(
            ...
            xi_max=300.0,
            n_points=1000,
            ra_hat_j=jnp.array([jnp.inf, jnp.inf]),
        )
