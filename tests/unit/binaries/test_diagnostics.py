...
def _circular_binary(a=1e-3, m1=2.0, m2=1.0):
    ...
    bs = KeplerElements(
        a=a, e=0.0, i=0.3, Omega=0.1, omega=0.2, M0=0.7
    ).to_binary_state(m1=m1, m2=m2, G=G)
...
class TestRelativeEnergy:
    ...
    def test_differentiable(self):
        ...
        r1, v1, r2, v2, m1, m2 = _circular_binary(a=1e-3)
        g = jax.grad(
            lambda a_scale: relative_energy(r1, r2 * a_scale, v1, v2, m1, m2, G=G)
        )(1.0)
...
class TestPrimordialSurvival:
    ...
    def test_disruption_counted(self):
        ...
        sec_idx = int(
            jnp.where((ic.primordial_system_id == 0) & ic.is_primordial_secondary)[0][0]
        )
...
def _isolated_binaries(K=3, a=1e-3, sep=100.0):
    ...
    for k in range(K):
        bs = KeplerElements(
            a=a, e=0.0, i=0.2, Omega=0.1, omega=0.3, M0=0.5
        ).to_binary_state(m1=1.0, m2=1.0, G=G)
        center = jnp.array([k * sep, 0.0, 0.0])
        com_vel = jnp.array(
            [0.0, float(k) * 10.0, 0.0]
        )  # distinct -> COM pairs unbound
...
class TestFindBoundMultiples:
    def test_isolated_binaries_multiplicity_2(self):
        ...
        system_id, multiplicity = find_bound_multiples(
            positions, velocities, masses, G=G
        )
