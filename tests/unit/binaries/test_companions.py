...
class TestIndependentCompanions:
    def test_shapes_and_singles(self):
        ...
        is_binary, el = _independent().sample(
            jax.random.PRNGKey(0), m1, G=G, day_in_time_units=DAY
        )
    ...
    def test_mass_dependent_q_dispatch(self):
        ...
        _, el_lo = model.sample(
            jax.random.PRNGKey(2), m_low, G=G, day_in_time_units=DAY
        )
        _, el_hi = model.sample(
            jax.random.PRNGKey(2), m_high, G=G, day_in_time_units=DAY
        )
    ...
    def test_jit(self):
        m1 = jnp.full(100, 1.5)
        f = jax.jit(
            lambda k, m: _independent().sample(k, m, G=G, day_in_time_units=DAY)
        )
...
class TestMoeCompanions:
    def test_shapes_singles_ranges(self):
        ...
        is_binary, el = MoeCompanions().sample(
            jax.random.PRNGKey(0), m1, G=G, day_in_time_units=DAY
        )
    ...
    def test_mass_dependent_binary_fraction(self):
        ...
        mc = MoeCompanions()
        ib_lo, _ = mc.sample(
            jax.random.PRNGKey(1), jnp.full(50000, 0.3), G=G, day_in_time_units=DAY
        )
        ib_hi, _ = mc.sample(
            jax.random.PRNGKey(1), jnp.full(50000, 20.0), G=G, day_in_time_units=DAY
        )
    ...
    def test_p_q_interrelation_in_secondary_masses(self):
        ...
        is_binary, el = MoeCompanions().sample(
            jax.random.PRNGKey(3), m1, G=G, day_in_time_units=DAY
        )
        bm = is_binary
        a = el.a[bm]
        q = el.m2[bm] / m1[bm]
        ...
        assert q_short > q_long, (
            f"q(short P)={q_short:.3f} should exceed q(long P)={q_long:.3f}"
        )
    ...
    def test_jit(self):
        ...
        m1 = jnp.full(500, 3.0)
        f = jax.jit(
            lambda k, m: MoeCompanions().sample(k, m, G=G, day_in_time_units=DAY)
        )
