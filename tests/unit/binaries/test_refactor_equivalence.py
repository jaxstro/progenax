...
def test_eccentricity_samplers_bit_identical():
    ...
    _assert_eq(
        "moe",
        LogisticThermalEccentricity().sample(k, jnp.array([5.0, 50.0, 500.0, 5000.0])),
    )
...
def test_orientation_radial_massdep_bit_identical():
    k = jax.random.PRNGKey(0)
    _assert_eq(
        "radial", RadialBinaryFraction().compute(jnp.array([0.1, 1.0, 5.0, 10.0]))
    )
