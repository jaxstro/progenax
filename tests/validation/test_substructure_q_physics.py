...
class TestCW04Baselines:
    ...
    def test_concentration_raises_Q(self):
        ...
        assert Qp > Qu and Qp > 0.80, (
            f"Plummer Q={Qp:.3f} should exceed uniform {Qu:.3f} and 0.80"
        )
    ...
    def test_full_ordering(self):
        ...
        assert Qc < Qu < Qp, (
            f"ordering failed: clumpy={Qc:.3f}, unif={Qu:.3f}, plummer={Qp:.3f}"
        )
...
class TestQNIndependence:
    def test_q_roughly_n_independent(self):
        ...
        for N in (150, 300, 600, 1200):
            Qs.append(
                np.mean([compute_q_parameter(_uniform_sphere(N, s)) for s in range(4)])
            )
        Qs = np.array(Qs)
...
class TestQApprox:
    ...
    def test_approx_preserves_ordering(self):
        ...
        assert qc < qu < qp, (
            f"q_approx ordering: clumpy={qc:.3f}, unif={qu:.3f}, conc={qp:.3f}"
        )
