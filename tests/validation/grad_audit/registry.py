...
def _king_r_t(W0):
    ...
    return jnp.atleast_1d(
        KingProfile.from_W0_rc(W0=W0, r_c=1.0, xi_max=400.0, n_ode_points=8000).r_t
    )
...
def _michie_r_t(W0):
    ...
    return jnp.atleast_1d(MichieProfile.from_W0_rc(W0=W0, r_c=1.0, r_a=_MICHIE_R_A).r_t)
...
def _eff_velocities_gamma(gamma):
    positions = EFFProfile(a=1.0, gamma=gamma, r_t=10.0).sample_positions(
        _MASSES, _KEY_POS
    )
...
def _powerlaw_cdf_mmin(m_min):
    return PowerLawIMF(
        exponents=[2.35], breakpoints=[], m_min=m_min, m_max=_PL_M_MAX
    ).cdf(_H4_QUERY)
...
_SEG_MASSES = jnp.concatenate(
    [jnp.full(_MASSES.shape[0] - _SEG_N_MASSIVE, 0.5), jnp.full(_SEG_N_MASSIVE, 10.0)]
)
...
def _cluster_sample_W0(W0):
    return (
        _cluster_engine_a(W0, 1.0, 0.5)
        .sample_cluster(_KEY, _CLUSTER_N_STARS, G=STELLAR.G)
        .positions
    )
...
def _cluster_sample_delta(delta):
    return (
        _cluster_engine_a(5.0, 1.0, delta)
        .sample_cluster(_KEY, _CLUSTER_N_STARS, G=STELLAR.G)
        .positions
    )
...
def _bc_plummer_om(r_a):
    ...
    return build_cluster(
        PlummerProfile(r_h=1.0), masses=_MASSES, key=_KEY, anisotropy_radius=r_a
    ).velocities
...
def _bc_plummer_omega(omega):
    ...
    return build_cluster(
        PlummerProfile(r_h=1.0), masses=_MASSES, key=_KEY, rotation=omega
    ).velocities
...
def _beff_gamma(gamma):
    ...
    return build_eff_cluster(
        masses=_MASSES, a=1.0, gamma=gamma, r_t=10.0, key=_KEY
    ).positions


def _bmich_W0(W0):
    # Michie family through the alias; fixed xi_max=800 -> W0 grid consistent for AD and FD.
    return build_michie_cluster(
        masses=_MASSES, W0=W0, r_c=1.0, r_a=8.0, key=_KEY
    ).positions


def _blim_W0(W0):
    # LIMEPY family through the alias; fixed xi_max=300 -> W0 grid consistent for AD and FD.
    return build_limepy_cluster(
        masses=_MASSES, W0=W0, g=1.0, r_c=1.0, key=_KEY
    ).positions
...
def _bcfp_rh(r_h):
    ...
    return build_cluster_from_params(
        ClusterParams(profile=PlummerProfile(r_h=r_h)), masses=_MASSES, key=_KEY
    ).positions
...
def _cluster_b_sample_ra(r_a):
    ...
    return (
        _cluster_engine_b(profiles, r_a_j=r_a_j)
        .sample_cluster(_KEY, _CLUSTER_N_STARS, G=STELLAR.G)
        .velocities
    )
...
_KEPLER_FROM_STATE_BASE = KeplerElements(
    a=1.0, e=0.3, i=0.3, Omega=0.4, omega=0.5, M0=1.0
).to_state(M_total=2.0, G=STELLAR.G)
...
def _kepler_from_state_vscale(s):
    return jnp.atleast_1d(
        KeplerElements.from_state(
            _KEPLER_R0, s * _KEPLER_V0, M_total=2.0, G=STELLAR.G
        ).a
    )
...
def _model_pk_rh(r_h):
    ...
    return profile.enclosed_mass_fraction(
        _BK_R_EDGES[1:]
...
_log10_mean = lambda periods: jnp.mean(
    jnp.log10(periods)
)  # the scattered-test observable
...
_DISP_R_A = (
    2.0  # OM anisotropy radius (>> 0.75 a ~ 0.575, well inside the valid domain)
)
...
def _df_moment_dispersion_W0(W0):
    ...
    return df_moment_dispersion(
        df, jnp.array([2.0, 13.0, 25.0]), 400.0, STELLAR.G
    ).sigma_r
...
def _jeans_dispersion_eff_gamma(gamma):
    ...
    return jeans_dispersion(
        EFFProfile(a=1.0, gamma=gamma, r_t=8.0),
        None,
        _DISP_R_SPAN_RT_JEANS,
        400.0,
        STELLAR.G,
    ).sigma_r
...
def _project_dispersion_eff_a(a):
    ...
    return project_dispersion(
        EFFProfile(a=a, gamma=5.0, r_t=8.0),
        None,
        _DISP_R_SPAN_RT_PROJ,
        400.0,
        STELLAR.G,
    ).sigma_los
...
REGISTRY: list[Case] = [
    Case(
        id="PlummerProfile.sample_positions",
        direction="params->IC",
        fn=_plummer_positions,
        param="r_h",
        theta0=1.0,
        reduce=mean_radius,
        expect="consistent",
    ...
    Case(
        id="PlummerVelocityDF.sample_velocities",
        direction="params->IC",
        fn=_plummer_velocities,
        param="r_h",
        theta0=1.0,
        reduce=mean_speed,
        expect="consistent",
    ...
    Case(
        id="PlummerVelocityDF+OM.sample_velocities",
        direction="params->IC",
        fn=_plummer_velocities_om,
        param="r_a",
        theta0=2.0,
        reduce=mean_speed,
        expect="consistent",
    ...
    Case(
        id="KingProfile.sample_positions",
        direction="params->IC",
        fn=_king_positions_W0,
        param="W0",
        theta0=7.0,
        reduce=mean_radius,
        expect="consistent",
    ...
    Case(
        id="KingVelocityDF.sample_velocities",
        direction="params->IC",
        fn=_king_velocities_W0,
        param="W0",
        theta0=7.0,
        reduce=mean_speed,
        expect="consistent",
    ...
    # --- Michie-King anisotropic profile + DF (Task 1.3) ---
    Case(
        id="MichieProfile.sample_positions",
        direction="params->IC",
        fn=_michie_positions_W0,
        param="W0",
        theta0=7.0,
        reduce=mean_radius,
        expect="consistent",
    ...
    Case(
        id="MichieVelocityDF.sample_velocities",
        direction="params->IC",
        fn=_michie_velocities_W0,
        param="W0",
        theta0=7.0,
        reduce=mean_speed,
        expect="consistent",
    ...
    Case(
        id="MichieProfile.density[log rho(r)]",
        direction="params->summary",
        fn=_michie_log_density_rc,
        param="r_c",
        theta0=1.0,
        reduce=identity_sum,
        expect="consistent",
    ...
    Case(
        id="EFFProfile.sample_positions",
        direction="params->IC",
        fn=_eff_positions_gamma,
        param="gamma",
        theta0=3.0,
        reduce=mean_radius,
        expect="consistent",
    ...
    Case(
        id="EFFVelocityDF.sample_velocities",
        direction="params->IC",
        fn=_eff_velocities_gamma,
        param="gamma",
        theta0=5.0,
        reduce=mean_speed,
        expect="consistent",
    ...
    Case(
        id="build_spatial_ic[Plummer]",
        direction="params->IC",
        fn=_build_spatial_ic_rh,
        param="r_h",
        theta0=1.0,
        reduce=mean_radius,
        expect="consistent",
    ...
    Case(
        id="build_spatial_ic[Plummer].velocities",
        direction="params->IC",
        fn=_build_spatial_ic_rh_velocities,
        param="r_h",
        theta0=1.0,
        reduce=mean_speed,
        expect="consistent",
    ...
    Case(
        id="PowerLawIMF.sample[Salpeter]",
        direction="params->IC",
        fn=_powerlaw_sample_alpha,
        param="alpha",
        theta0=2.35,
        reduce=mean_mass,
        expect="consistent",
    ...
    Case(
        id="ChabrierIMF.ppf[H6 boundary]",
        direction="params->IC",
        fn=_chabrier_ppf_mc_boundary,
        param="m_c",
        theta0=0.08,
        reduce=identity_sum,
        expect="consistent",
    ...
    Case(
        id="PowerLawIMF.cdf[H4]",
        direction="params->IC",
        fn=_powerlaw_cdf_mmin,
        param="m_min",
        theta0=_PL_M_MIN,
        reduce=identity_sum,
        expect="consistent",
    ...
    Case(
        id="PowerLawIMF.ppf[m_min]",
        direction="params->IC",
        fn=_powerlaw_ppf_mmin,
        param="m_min",
        theta0=_PL_M_MIN,
        reduce=identity_sum,
        expect="consistent",
    ...
    Case(
        id="lambda_msr_approx",
        direction="params->summary",
        fn=_lambda_msr_core_scale,
        param="core_scale",
        theta0=0.3,
        reduce=lambda x: jnp.sum(jnp.atleast_1d(x)),
        expect="consistent",
    ...
    Case(
        id="MultiComponentCluster.sample_cluster[EngineA]",
        direction="params->IC",
        fn=_cluster_sample_W0,
        param="W0",
        theta0=5.0,
        reduce=mean_radius,
        expect="consistent",
    ...
    Case(
        id="MultiComponentCluster.from_components[EngineA]",
        direction="params->IC",
        fn=_cluster_a_w_j,
        param="w_j",
        theta0=0.8,
    ...
    Case(
        id="MultiComponentCluster.from_mass_segregation[EngineA]",
        direction="params->IC",
        fn=_cluster_a_r_a,
        param="r_a",
        theta0=4.0,
    ...
    Case(
        id="SanaOBPeriod.sample",
        direction="params->IC",
        fn=_sana_period_power,
        param="power",
        theta0=-0.55,
        reduce=_log10_mean,
        expect="consistent",
    ...
    Case(
        id="LogNormalPeriod.sample",
        direction="params->IC",
        fn=_lognormal_period_mu,
        param="mu_log_P",
        theta0=4.8,
        reduce=_log10_mean,
        expect="consistent",
    ...
    Case(
        id="LogUniformPeriod.sample",
        direction="params->IC",
        fn=_loguniform_period_logpmax,
        param="log_P_max",
        theta0=8.0,
        reduce=_log10_mean,
        expect="consistent",
    ...
    Case(
        id="ThermalEccentricity.sample",
        direction="params->IC",
        fn=_thermal_ecc_emax,
        param="e_max",
        theta0=0.99,
        reduce=_mean_ecc,
        expect="consistent",
    ...
    Case(
        id="UniformEccentricity.sample",
        direction="params->IC",
        fn=_uniform_ecc_emax,
        param="e_max",
        theta0=0.9,
        reduce=_mean_ecc,
        expect="consistent",
    ...
    Case(
        id="LogisticThermalEccentricity.sample",
        direction="params->IC",
        fn=_logistic_thermal_ecc_emax,
        param="e_max",
        theta0=0.99,
        reduce=_mean_ecc,
        expect="consistent",
    ...
    Case(
        id="IndependentCompanions.sample",
        direction="params->IC",
        fn=_independent_companions_e_max,
        param="e_max",
        theta0=0.99,
        reduce=_mean_ecc,
        expect="consistent",
    ...
    Case(
        id="MultiComponentCluster.sample_cluster[EngineB]",
        direction="params->IC",
        fn=_cluster_b_sample_rh,
        param="r_h",
        theta0=2.0,
        reduce=mean_radius,
        expect="consistent",
    ...
    Case(
        id="KeplerElements.to_state",
        direction="params->IC",
        fn=_kepler_to_state_e,
        param="e",
        theta0=0.5,
        reduce=mean_radius,
        expect="consistent",
    ...
    Case(
        id="KeplerElements.from_state",
        direction="params->IC",
        fn=_kepler_from_state_vscale,
        param="v_scale",
        theta0=1.0,
        reduce=identity_sum,
        expect="consistent",
    ...
    Case(
        id="resolve_binary_components",
        direction="params->IC",
        fn=_resolve_binary_a,
        param="a",
        theta0=0.5,
        reduce=mean_radius,
        expect="consistent",
    ...
    Case(
        id="resolve_binary_components[mixed]",
        direction="params->IC",
        fn=_resolve_binary_a_mixed,
        param="a",
        theta0=0.5,
        reduce=mean_radius,
        expect="consistent",
    ...
    Case(
        id="MoeCompanions.sample",
        direction="params->IC",
        fn=_moe_companions_mean_e,
        param="m1_scale",
        theta0=1.0,
        reduce=lambda x: jnp.sum(jnp.atleast_1d(x)),
        expect="consistent",
    ...
    Case(
        id="apply_solid_body_rotation",
        direction="params->IC",
        fn=_solid_body_omega,
        param="omega",
        theta0=0.5,
        reduce=mean_speed,
        expect="consistent",
    ...
    Case(
        id="apply_differential_rotation",
        direction="params->IC",
        fn=_differential_v_peak,
        param="v_peak",
        theta0=1.0,
        reduce=mean_speed,
        expect="consistent",
    ...
    Case(
        id="binned_sigma1d[Plummer]",
        direction="params->summary",
        fn=_binned_sigma1d_rh,
        param="r_h",
        theta0=1.0,
        reduce=identity_sum,
        expect="consistent",
    ...
    Case(
        id="binned_sigma_beta[Plummer+OM]",
        direction="params->summary",
        fn=_binned_beta_ra,
        param="r_a",
        theta0=_BK_OM_R_A,
    ...
    Case(
        id="PlummerProfile.enclosed_mass_fraction[N(r) model]",
        direction="params->summary",
        fn=_model_pk_rh,
        param="r_h",
        theta0=1.0,
    ...
    Case(
        id="binned_number_density[data, pinned non-diff]",
        direction="params->summary",
        fn=_binned_number_density_rh,
        param="r_h",
        theta0=1.0,
        reduce=identity_sum,
    ...
    Case(
        id="build_cluster[Plummer+OM]",
        direction="params->IC",
        fn=_bc_plummer_om,
        param="anisotropy_radius",
        theta0=0.7,
        reduce=mean_speed,
        tol=1e-3,
    ),  # 4.7e-6
    Case(
        id="build_cluster[Plummer+rotation]",
        direction="params->IC",
        fn=_bc_plummer_omega,
        param="omega",
        theta0=0.3,
        reduce=mean_speed,
        tol=1e-4,
    ...
    Case(
        id="build_cluster_from_params[ClusterParams]",
        direction="params->IC",
        fn=_bcfp_rh,
        param="r_h",
        theta0=1.0,
    ...
    Case(
        id="inverse_zams_luminosity",
        direction="params->summary",
        fn=_inverse_zams_L,
        param="L_target",
        theta0=_ZAMS_INVERSE_L,
        reduce=identity_sum,
        tol=1e-5,
    ),  # |ratio-1|=1.9e-9
    ...
    Case(
        id="jeans_dispersion[Plummer+OM]",
        direction="params->summary",
        fn=_jeans_dispersion_om_r_a,
        param="r_a",
        theta0=_DISP_R_A,
        reduce=identity_sum,
        expect="consistent",
    ...
    Case(
        id="jeans_dispersion[Plummer]",
        direction="params->summary",
        fn=_jeans_dispersion_M,
        param="M",
        theta0=400.0,
        reduce=identity_sum,
        expect="consistent",
    ...
    Case(
        id="project_dispersion[Plummer+OM]",
        direction="params->summary",
        fn=_project_dispersion_om_r_a,
        param="r_a",
        theta0=_DISP_R_A,
        reduce=identity_sum,
        expect="consistent",
    ...
    Case(
        id="project_dispersion[Plummer+OM].pm_t",
        direction="params->summary",
        fn=_project_dispersion_om_r_a_pmt,
        param="r_a",
        theta0=_DISP_R_A,
        reduce=identity_sum,
        expect="consistent",
    ...
    Case(
        ...
        direction="params->summary",
        fn=_df_moment_dispersion_W0,
        param="W0",
        theta0=6.0,
        reduce=identity_sum,
        expect="consistent",
        tol=1e-3,
    ),  # measured |ratio-1|~3e-6
    ...
    Case(
        id="jeans_dispersion[EFF spans r_t]",
        direction="params->summary",
        fn=_jeans_dispersion_eff_gamma,
        param="gamma",
        theta0=4.0,
        reduce=identity_sum,
        expect="consistent",
    ...
    Case(
        id="project_dispersion[EFF spans r_t]",
        direction="params->summary",
        fn=_project_dispersion_eff_a,
        param="a",
        theta0=1.0,
        reduce=identity_sum,
        expect="consistent",
