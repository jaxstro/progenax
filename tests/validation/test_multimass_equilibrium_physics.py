...
def test_per_group_virial_is_half_seed_averaged():
    """Per-group virial Q_j ~ 0.5 across delta (the equilibrium property the lambda_seg
    blend lacks per group). Light + global are tight; the rarer concentrated heavy
    component is softening/N-limited, so a wider band -- still clearly virial, and the
    drift is monotonic in concentration (a measurement effect, not a physical one)."""
    for delta in (0.0, 0.3, 0.6):
        model = _model(delta)
        Ql, Qh = [], []
        for s in range(5):
            ...
            Qj = np.asarray(per_group_virial_ratio(p, v, m, G=G, group_masks=masks, softening=SOFT))
            Ql.append(Qj[0]); Qh.append(Qj[1])
        assert abs(np.mean(Ql) - 0.5) < 0.05, f"delta={delta}: light Q={np.mean(Ql):.3f}"
        assert abs(np.mean(Qh) - 0.5) < 0.10, f"delta={delta}: heavy Q={np.mean(Qh):.3f}"
