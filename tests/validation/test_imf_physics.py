...
class TestSalpeterSlope:
    ...
    def test_salpeter_mean_mass(self, key):
        """Salpeter mean mass is in reasonable range."""
        alpha = 2.35
        ...
        # For Salpeter with alpha > 2, mean is dominated by low-mass end
        # Typical range: 0.2-0.6 M_sun depending on mass limits
        assert 0.1 < mean_computed < 1.0, \
            f"Salpeter mean mass = {mean_computed:.4f} (expected 0.1-1.0)"

        # Verify sample mean matches computed mean
...
class TestChabrierParameters:
    ...
    def test_chabrier_high_mass_slope(self, imf_constants):
        """Chabrier high-mass slope = Salpeter (2.35)."""
        imf = ChabrierIMF()
