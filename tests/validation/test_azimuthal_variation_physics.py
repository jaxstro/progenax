...
class TestAzimuthalVariation:
    def test_smooth_near_poisson_floor(self):
        ...
        N = 3000
        vals = [
            compute_azimuthal_variation(_smooth(N, s), n_bins=N_BINS) for s in range(8)
        ]
        floor = np.sqrt(N_BINS / N)
        mean = np.mean(vals)
        assert mean < 3 * floor, (
            f"smooth sigma_Sigma/<Sigma>={mean:.3f} vs floor {floor:.3f}"
        )
    ...
    def test_clumps_raise_metric(self):
        ...
        cl = np.mean(
            [compute_azimuthal_variation(_azimuthal_clumps(2000, s)) for s in range(5)]
        )
        assert cl > 3 * sm and cl > 0.3, (
            f"clumpy={cl:.3f} should exceed smooth={sm:.3f}"
        )
    ...
    def test_monotonic_with_clumpiness(self):
        ...
        vals = [
            np.mean(
                [
                    compute_azimuthal_variation(_azimuthal_clumps(2000, s, frac=f))
                    for s in range(4)
                ]
            )
            for f in fracs
    ...
    def test_spans_kupper_range(self):
        ...
        cl = np.mean(
            [
                compute_azimuthal_variation(
                    _azimuthal_clumps(2000, s, n_clumps=3, frac=0.85, width=0.1)
                )
                for s in range(5)
            ]
        )
        assert sm < 0.2 and cl > 0.5, (
            f"range not spanned: smooth={sm:.3f}, clumpy={cl:.3f}"
        )
    ...
    def test_anticorrelates_with_cw04_Q(self):
        ...
        assert corr < -0.5, (
            f"sigma_Sigma vs Q correlation = {corr:.2f} (expected strongly negative)"
        )
