...
PROVENANCE: dict[str, str] = {
    # ============================ Spatial profiles ===================================
    "profiles/plummer.py::scale-radius a = r_h*sqrt((1-0.5^(2/3))/0.5^(2/3))": "Plummer (1911) MNRAS 71, 460 — closed-form inverse of the Plummer mass CDF "
    "M(<r_h)/M=0.5; algebraically exact (= r_h*sqrt(2^(2/3)-1)). Ledger Batch-7 LOW "
    "spot-sample: canonical form, NOT the inverted-a bug.",
    "profiles/king.py::King(1966) Table II xi_t = r_t/r_c (c=4.699/131.4/2272 @ W0=3/9/15)": "King (1966) AJ 71, 64, Table II (p. 73) — verified Batch-3 against the held PDF; "
    "re-confirms log c {0.672/1.029/1.528/2.119}, NOT the debunked 0.84/1.18/1.48/1.76.",
    "profiles/limepy.py::LIMEPY density index g+3/2": "Gieles & Zocchi (2015) MNRAS 454, 576, Eq. 8 (p. 578) + erratum Eqs. 20/21 — "
    "index verified Batch-3 (main-text Eq. 8 = E_gamma(g+3/2, phi_hat); King g=1 corner).",
    "profiles/eff.py::EFF density rho(0)=1, rho(a)=2^(-gamma/2), rho~r^(-gamma)": "Elson, Fall & Freeman (1987) ApJ 323, 54 — closed-form EFF surface-density profile; "
    "derivable identity (asserted in tests/validation/test_eff_physics.py).",
    # ============================ IMFs (classic) =====================================
    "imf/power_law.py::Salpeter alpha = 2.35": "Salpeter (1955) ApJ 121, 161 (p. 165) — xi ∝ m^-1.35 per dlog m => alpha=2.35 "
    "per dm. Ledger Batch-5 ✅ verified against the held scanned PDF.",
    "imf/power_law.py::Kroupa exponents [0.3, 1.3, 2.3] + breaks [0.08, 0.5]": "Kroupa (2001) MNRAS 322, 231, Eq. 2 (p. 234, verbatim) — alpha0=0.3, alpha1=1.3, "
    "alpha2=alpha3=2.3; the 3-segment merge of alpha2=alpha3=2.3 is exact. Batch-5 ✅.",
    "imf/chabrier.py::m_c=0.08, sigma=0.69, A_ln=0.158, alpha=2.3": "Chabrier (2003) PASP 115, 763, Table 1 (p. 769) — single-star (disk) IMF: m_c=0.079 "
    "(≈0.08), sigma=0.69, A_ln=0.158, high-mass x=1.3 => alpha=2.3. Correctly distinct "
    "from the SYSTEM IMF (m_c≈0.2/sigma≈0.6). Batch-5 ✅.",
    "imf/smooth.py::Maschberger alpha=2.3, beta=1.4, mu=0.2, m_l=0.01": "Maschberger (2013) MNRAS 429, 1725, Table 1 (p. 1727, verbatim). Batch-5 ✅.",
    # ============================ Environment-dependent IMF (Marks/Jerabkova) ========
    "imf/environment/coefficients.py::JERABKOVA logMecl_coeff=0.6039, constant=0.2161": "Jerabkova et al. (2018) A&A 620, A39, Eq. 7/9 — 8pi half-mass-density convention: "
    "0.6039 = 0.99*0.61 (exact); 0.2161 is 8pi-derived (Batch-1 ✅; 8pi deviation from "
    "the Eq.9-printed 2.83 ratified 2026-06-09).",
    "imf/environment/coefficients.py::JERABKOVA x_threshold=-0.87, slope=-0.41, intercept=1.94, canonical=2.3": "Jerabkova (2018) Eq. 6 (cites the Marks 2014 erratum) — alpha3=2.3 (x<-0.87), "
    ...
    "imf/environment/coefficients.py::JERABKOVA FeH_coeff=-0.14, rho_logRho_coeff=0.99": "Jerabkova (2018) Eq. 7 (density-based, no constant term). Batch-1 ✅.",
    "imf/environment/coefficients.py::MARKS cos_theta=-0.139, sin_theta=0.990 (theta=98deg)": "Marks et al. (2012) MNRAS 422, 2246, Eq. 14/15 (p. 2252) — Fundamental-Plane "
    "rotation cos/sin(98deg). Batch-1 ✅.",
    "imf/environment/coefficients.py::MARKS slope=-0.4072, intercept=1.9383, x_hat_threshold=-0.87": "Marks (2012) Eq. 15 (full-precision 2012 slope/intercept; the 2014 erratum rounds "
    "to -0.41/1.94 while fixing the sign). Threshold -0.87 is the erratum-corrected value "
    "(was +0.87, a printed typo). Batch-1 ✅.",
    "imf/environment/coefficients.py::MARKS lowmass_slope=0.5": "Marks (2012) Eq. 12 (low-mass slope delta_alpha per [Fe/H]). Batch-1 ✅.",
    "imf/environment/coefficients.py::MARKS_TABLE3 mcl/mecl/rho/feh 1-D relations": "Marks (2012) Table 3 (p. 2251) — matches cell-for-cell: M_cl(-0.94/2.14/0.68), "
    "M_ecl(-0.77/1.59/0.27), rho_cl(-0.43/1.86/0.095), [Fe/H](0.66/2.63/<-0.5). Batch-1 ✅.",
    "imf/environment/coefficients.py::DEFAULT_SFE=0.33": "Jerabkova (2018) fiducial star-formation efficiency epsilon = 0.33. Batch-1.",
    "imf/environment/mapping.py::alpha3 clip [0.5, 2.3] + canonical 2.3": "Jerabkova (2018) Eq. 6 / Kroupa (2001) canonical high-mass slope 2.3; the clip "
    "bounds alpha3 to the physical [0.5, canonical] range. Batch-1 ✅ (consumer of "
    "coefficients.py).",
    # ============================ Binaries: Moe & Di Stefano (2017) ==================
    "imf/binary/moe_di_stefano.py::Table 13 grids (gamma_largeq, gamma_smallq, F_twin, f_logP)": "Moe & Di Stefano (2017) ApJS 230, 15, Table 13 (p. 52) — ALL 80 cells match exactly "
    "(incl. the '<0.03 -> 0' twin convention and the -1.1/-2.0 tail). Batch-2 ✅.",
    "imf/binary/moe_di_stefano.py::_MASS_NODES [1.0,3.2,6.7,12,20], _LOGP_NODES [1,3,5,7]": "Moe (2017) Table 13 representative mass bins (Solar/A-lateB/MidB/EarlyB/O) and "
    "logP rows. Batch-2 ✅ (the interpolation node grid of Table 13).",
    "binaries/eccentricity.py::Moe eta Eq.17 (0.6-0.7/(logP-0.5)) + Eq.18 (0.9-0.2/(logP-0.5))": "Moe & Di Stefano (2017) Eqs. 17/18 — late-type (0.8<M1<3 Msun) and early-type "
    "(M1>7 Msun) eccentricity power-law index eta(logP). Batch-2 ✅ (reproduces Table 13 eta).",
    # ============================ Binaries: Sana / Lucy / mass-ratio =================
    "binaries/period.py::Sana pi=-0.55, range logP in [0.15, 3.5]": "Sana et al. (2012) Science 337, 444 — OB period distribution p(logP) ∝ (logP)^pi, "
    ...
    "imf/binary/mass_ratio.py::Sana q-slope kappa ≈ -0.1": "Sana et al. (2012) Science 337, 444 — main text & Fig. 1 (Science Report has NO "
    "numbered equations; corrected from the stale 'Eq. 3' cite). Batch-5 🔧 fixed.",
    "imf/binary/mass_ratio.py::TwinPeaked f_twin=0.1, sigma_twin=0.03": "Lucy (2006) A&A 457, 629 — systematic statistical study of the q≈1 twin excess "
    ...
    "diagnostics/substructure.py::CW04 Table 1 radial Q (3D0/3D1/3D2 = 0.79/0.84/0.93)": "Cartwright & Whitworth (2004, CW04) MNRAS 348, 589, Table 1 (p. 590) — exact (3D1 "
    ...
    "diagnostics/segregation_approx.py::Sigma=(k-1)/(pi r_k^2), k=6": "von Hoerner (1963) / Casertano & Hut (1985), as adopted by Maschberger & Clarke "
    ...
    "builders.py::compute_stellar_radii 1.06/0.945, 1.33/0.555, knee 1.66": "Demircan & Kahraman (1991) Ap&SS 181, 313, Table II (EMPIRICAL R=10^a M^b; "
    ...
    "stellar.py::_TOUT_L_COEFFS (Tout+1996 Table 1, L(M,Z))": "Tout, Pols, Eggleton & Han (1996), MNRAS 281, 257, Table 1 (eq. 3) — 35/35 cells "
    "verified cell-by-cell vs held PDF; see "
    "docs/core-papers/tout1996_zams_coefficients_verified.md + provenance-ledger.md.",
    "stellar.py::_TOUT_R_COEFFS + _TOUT_R_NU (Tout+1996 Table 2, R(M,Z))": "Tout, Pols, Eggleton & Han (1996), MNRAS 281, 257, Table 2 (eq. 4) — 40 cells + "
    ...
    "analytical/base.py::planet/Sun mass ratios (IAU 2009 / Luzum et al. 2011 Table 1)": "IAU (2009) Current Best Estimates / Luzum et al. (2011) Cel. Mech. Dyn. Astron. "
    "109, 293, Table 1 — all 8 M_sun/M_planet reciprocals verified (Earth 332946.05; "
    "Jupiter 1.047348644e3; etc.). Batch-4 ✅.",
    "analytical/base.py::planet orbital elements (a,e,inc,Omega,omega)": "Standish & Williams (2012) / JPL Horizons J2000 — source NOT held; accepted as "
    "standard J2000 (Anna's Batch-4 call; noted source-not-held in the code comment).",
    "analytical/few_body.py::figure-eight period T0 = 6.32591398": "Chenciner & Montgomery (2000) Ann. Math. 152, 881 — dimensionless (G=1, m=1, "
    ...
    "kinematics/dispersion.py::anisotropic-Jeans sigma_r quadrature (OM integrating factor r^2+r_a^2)": "Binney & Tremaine (2008) Galactic Dynamics sec. 4.8.3 (anisotropic Jeans) + Merritt "
    "(1985) AJ 90, 1027 (Osipkov-Merritt anisotropy, sigma_r^2/sigma_t^2 = 1 + r^2/r_a^2) "
    "— derivable identity. (Specific equation number not re-verified vs the PDF in-session.)",
    "kinematics/dispersion.py::Binney & Mamon (1982) LOS projection kernels + isotropic-Plummer sigma_los oracle": "Binney & Mamon (1982) MNRAS 200, 361 (projection integrals); isotropic-Plummer "
    "sigma_los^2 = (3 pi/64) G M / sqrt(a^2+R^2) per Dejonghe (1987) MNRAS 224, 13 — "
    "both derivable/cited identities.",
    "kinematics/dispersion.py::Michie-King DF second-moment quadrature (df_moment_dispersion)": "Michie (1963) MNRAS 125, 127 (anisotropy exp(-beta J^2), Eq 4.0/4.2) + King (1966) "
