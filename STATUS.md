...
next: Validation-audit campaign COMPLETE (stated scope + the azimuthal-variation gap). ONE final PR for the 6-commit campaign is the natural next step (per sota-pass workflow, CI green) — awaiting Anna's go-ahead (push is outward-facing). Optional later: out-of-scope ⚠️ dashboard rows (imf-statistics, binary-imf, analytical-test-cases) to figure-standard. gravoturb_fdf/pp20 deferred by instruction. compute_azimuthal_variation now figure-validated (commit ed2898b, released-core 861→866).
