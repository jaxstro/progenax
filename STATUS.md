...
next: Validation-audit campaign COMPLETE for the stated scope (spatial profiles + kinematics + substructure + diagnostics). Decide: (a) raise the ONE final PR for the 5-commit campaign (per sota-pass workflow, CI green); (b) optional minor gap — figure-validate compute_azimuthal_variation (Küpper 2011), the only unfigured released diagnostic; (c) out-of-scope ⚠️ dashboard rows (imf-statistics, binary-imf, analytical-test-cases) if you want them brought to figure-standard later. gravoturb_fdf/pp20 deferred by instruction.
