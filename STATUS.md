...
next: **AWAITING HITL: differentiable mass-segregation observables (milestone A) on `feat/differentiable-segregation` — ready to merge + ONE PR.** New released-core `progenax.diagnostics.segregation_approx`: three differentiable segregation *observables* (soft Λ_MSR / radial concentration / Σ–m) sharing a sigmoid soft-mass-cut kernel w_i=σ((m_i−m_cut)/τ). Closes the gap the mass-segregation page flagged — a differentiable *observable* (f(positions,masses)→ℝ with usable ∂f), distinct from the already-differentiable forward-model *parameter* `lambda_seg` (the conflation that confused us; §4 prose now fixed). TDD throughout: +27 unit (tests/unit/diagnostics/) +13 validation (Oracles 1–4: soft→exact hard-limit [radial/Σ to ~1e-11 via jnp.sort exact k-NN radius; λ-NN to 5.6e-3], monotonic+Spearman 0.98 vs Allison Λ_MSR oracle, regimes, AD-vs-FD ~1e-10). scripts/validate_segregation_approx.py 5 figs ALL PASS incl. **Fisher-information identifiability** (radial ~5× most identifiable, I=649 vs 132/143 → recommended HMC summary) + **2D-vs-3D projection** Fisher-ratio (radial keeps ~50%, Σ–m ~32%, local-NN λ unaffected). Design: docs/plans/2026-06-09-differentiable-segregation-observable-design.md. lambda_msr_approx softmin uses scale-relative β (stop_gradient median-NN → size-invariant). mass-segregation.md §5+Measured col, unicode→KaTeX, build-verified (figs in public/+JSON). Released-core 893→934 (full suite, exit 0). **Deferred to milestone B (in design doc + page admonition): data-space inference, noisy mass proxy, learned 2D→3D deprojection.** Prior campaign-complete state otherwise holds (gravoturb_fdf/pp20 still ⚠️ experimental). Commits on branch: design, kernel+radial, lambda, sigma, calibration+exports, validation+figs, docs.
